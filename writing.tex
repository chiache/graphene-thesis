\ifx \debug \undefined

\newcommand{\issue}[2]{{}}
\newcommand{\issuedone}[2]{{}}

\newcommand{\issuepage}{{}}

\newcommand{\keepthis}[1]{{#1}}
\newcommand{\polish}[1]{{#1}}
\newcommand{\rewrite}[1]{{#1}}

\else

\newcommand{\issue}[3][red!40]{{\todo[caption=\parbox{8em}{Issue #2}\parbox{6.5em}{\thechapterorsection:} {#3}, fancyline, color=#1, list]{
  \begin{spacing}{1.0}
    \lsstyle
    \tt Issue #2:\\ #3\\
    \vspace*{-0.5\baselineskip}
  \end{spacing}
}}\hspace{-.25em}} 

\newcommand{\issuedone}[2]{\issue[green!40]{#1 (done)}{#2}}

\newcommand{\issuepage}{%
    \cleardoublepage\phantomsection
    \addcontentsline{toc}{chapter}{Writing Plan}

    \noindent
    {\Huge\bf Writing Plan}
    \vspace*{2\baselineskip}
    
    \noindent
    {\large Access the following Google Doc:}\\
    \parbox{\textwidth}{\normalsize\url{https://docs.google.com/a/cs.stonybrook.edu/document/d/1Wk4dk2mgFPXlB_3hRwfPiyQdBI16B9eT4xA5qpJ24iA/edit?usp=sharing}}
    
    \vspace*{-4\baselineskip}
    
    \begingroup
    \small
    \let\cleardoublepage\relax
    \let\clearpage\relax
    \setstretch{1.2}
    \listoftodos[]
    \endgroup
    
    \newpage
}

\usepackage{xcolor}

\newcommand{\hlcolor}[2][yellow]{%
  \textcolor{#1}{{#2}}%
}

\definecolor{forestgreen}{rgb}{0.03, 0.47, 0.19}
\newcommand{\keepthis}[1]{\hlcolor[forestgreen]{{#1}}}
\newcommand{\polish}[1]{\fixme{polish}\hlcolor[blue]{{#1}}}
\definecolor{darkred}{rgb}{0.81, 0.06, 0.13}
\newcommand{\rewrite}[1]{\fixme{rewrite this}\hlcolor[darkred]{{#1}}}

\fi