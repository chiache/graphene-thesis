...
which can be
\fixme{Just prediction; the actual number needs to be collected.}
20--30\% slower than an application-level NFS client on Linux (using \code{libnfs}).
...
A guest-level file system can potentially introduce a pre-formatted partition or virtual disk into \thelibos{}.
...
to decompose a pre-formated partition or virtual disk.
The drawback of using a pre-formated partition or virtual disk
is the complexity of coordinating multiple \picoprocs{} which access the same file system.
Therefore, the usage of a pre-formated partition or virtual disk
is mostly restricted to a single-process application.
