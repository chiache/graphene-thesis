...
However, \graphene{} does not adopt the technique of live VM migration
as copying the whole virtual address space of the \picoproc{}.
Instead, the checkpoint scheme of \thelibos{}
locks each handle when copying it to the heap or over a RPC stream.
\graphene{} chooses not to copy the whole virtual address space for three reasons.
First, a checkpoint scheme that snapshots the whole \picoproc{} memory
cannot differentiate temporary and permanent states inside a \libos{}.
To improve I/O performance, \thelibos{} tends to reserve a significant amount of virtual memory for caching and buffering, and thus \thelibos{} can reduce migration time by skipping these temporary states.
Second, by checkpointing handles individually,
\thelibos{} can overwrite each handle and sanitize any sensitive states before sending the snapshot out to another \picoprocs{}.
Finally, \thehostabi{} does not export any functions for tracing
the dirty pages during live migration,
since this feature requires low-level hardware support
and is difficult to port on a restricted hardware platform like Intel SGX.
For all the reasons above,
\thelibos{} only selectively checkpoints handles and \libos{} states, instead of snapshotting the whole \picoproc{} memory.
...
To reduce the overhead of migrating process states over a RPC stream, \graphene{} introduces a {\bf bulk IPC} mechanism in \thehostabi{}.
