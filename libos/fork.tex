...
As with a virtual machine, \graphene{} creates each new \picoproc{} in a ``clean'' state, along with an individual \thelibos{} instance. % fork is implemented in the \libos{}.
...
\thelibos{} implements \syscall{fork} by leveraging process checkpoint and migration.
...
and restores the states in the new \picoproc{}. % to create a forked process. 



A simple but naive way of checkpointing an application process
is to snapshot the whole virtual address space
of a \picoproc{}, including memory mapping of both the application and \thelibos{}.
