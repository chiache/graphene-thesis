...
The specification of \syscall{execve} includes detaching a thread from a multi-threaded process and moving it to a clean, new process.
Unlike the behavior in a single-threaded process,
\syscall{execve} cannot replace the executable in a multi-threaded process since other running threads still run in the same context.
...
\thelibos{} implements \syscall{execve} by partially migrating the \libos{} states
to a new \picoproc{}.
The specification of \syscall{execve} requires the OS to reset
...
opened without the \code{CLOEXEC} flag.
\thelibos{} uses the same checkpoint and migration scheme
to migrate the process state for \syscall{execve},
but only includes the current thread handle, all the non-\code{CLOEXEC} file handles, and the global OS states (e.g., namespace information) in the checkpoint data.
