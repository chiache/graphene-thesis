...
the attackers often try to corrupt a pointer on the stack to make it points to a piece of security-sensitive code or data.
With ASLR, a Linux kernel increases the unpredictability of memory mappings,
so that it becomes harder for a remote attacker
to pinpoint the memory target.
...
\thelibos{} adds a random factor to the procedure of determining the VMA addresses.
...
\thelibos{} also randomizes the results of \syscall{brk};
\thelibos{} reserves a random-sized gap between the executable's data segment and the brk region.
