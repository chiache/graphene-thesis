...
The I/O abstractions are generally simple byte streams.
...
and can utilize different types of I/O devices through unified file system APIs such as \syscall{read} and \syscall{write}.
...
For example, a append-only file can only be opened with the \palkeyword{APPEND} mode.
...
If \palkeyword{TRY_CREATE} is given,
...
The semantics of \palcall{StreamOpen} belong to a subset of POSIX's \syscall{open} semantics, which coincide with \win{}'s \syscall{OpenFile} semantics, with parameter options available in both host OSes.
...
More low-level abstractions such as file system blocks or raw sockets are mostly needed by administrative-type applications such as \code{fsck} and \code{ipconfig}, which are less desired for porting.
Other common parameters to opening a file or network socket
are either emulatable in the \libos{} (e.g., asynchronous I/O and close-on-\syscall{execve}) or not directly related with the usability of applications (e.g., server backlog sizes).
...
supported by \palcall{StreamOpen} are simply byte streams.
\palcall{StreamRead} and \palcall{StreamWrite} easily cover all access types on the I/O streams,
...
At success, \palcall{StreamMap} returns the mapped address; otherwise, a null address is returned.
...
should be defined in \palkeyword{STREAM_ATTRS}.
...
Potentially \palcall{StreamSetLength} can be absorbed by \palcall{StreamAttrSetByHandle}. but currently the \hostapi{} is preserved as a legacy from previous version of \thehostabi{} and as an optimization for file operations which need to frequently update file sizes (\palcall{StreamAttrSetByHandle} is much slower than \palcall{StreamSetLength}).
