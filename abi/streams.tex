...
\palcall{StreamMap} maps a file to an address in memory, for reading and writing data, or executing code stored in a binary file.
...
Finally, \palcall{StreamSetLength} extends or truncates the size of a regular file.
The \palkeyword{size} given to \palcall{StreamSetLength} can either be larger than the current size of the file, to allocate more space on the storage,
or be smaller than the current size, to shrink the space taken on the storage.
Although \palcall{StreamWrite} will automatically extend a file when writing beyond the end of file,
a file-backed memory mapping created by \palcall{StreamMap} cannot extend the file
when writing to memory pages that are not backed by file content.
When memory access exceeds the file boundary, the host OS raises a memory fault.
\palcall{StreamSetLength} allows the guest to extend a file before proceeding to access a memory mapping beyond the end of file, or to truncate the file to proper length after the access is finished.
