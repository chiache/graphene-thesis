...
but can be given to \palcall{StreamWaitForClient} to block and receive a client connection.
...
\palcall{StreamOpen} defines the scope of enforcing and configuring security isolation in the host OSes.
URIs for \palcall{StreamOpen} represent three types of host resources that are shareable among guests:
file, network, and RPC. 
Other host resources, such as threads and memory,
are local to each process, and thus can be isolated by pinning the resources.
Restricting the shareable host resource to I/O streams simplifies
the enforcement of security policies in the hosts.
In the host ABI, \palcall{StreamOpen} is the only \hostapi{} that requires permission checks in the host OS.
A user can configure the security policies of sharing these I/O stream
by simply listing URIs that are permitted for an application.
