This chapter specifies the formal definition of the host ABI as a component
...
\polish{The host ABI acts as a boundary between the host and the guest (i.e., the \libos{}),
and defines several UNIX-like features. % for \libos{} development.
The specification of the host ABI is primarily based on two criterions:
{\em simplicity}, for reducing the development effort per host, and {\em sufficiency}, for encapsulating enough host-specific system abstractions.}
The chapter discusses
the rationale behind the definition of the current host ABI,
according to the experience of porting the host ABI to several host examples, such as Linux, Windows, and SGX.
...
\section{PAL Calling Convention}
...
The PAL contains a simple run-time loader that can load the library OS as an ELF (executable and linkable format) binary~\cite{elf-format}, similar to \code{ld.so} for loading a user library.
