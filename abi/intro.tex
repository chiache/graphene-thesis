...
The host ABI presents a specification of which OS and hardware abstractions should be implemented in the hosts, %, in order to implement the library OS.
and how to develop
...
The design of the host ABI is to load the library OS as a user library.
...
The PAL also works as a dynamic loader, similar to \code{ld.so},
...
The host ABI assumes the library OS to be developed as a Linux-compliant, ELF (extensible linking format) binary.
%The host ABI inherit the Linux calling convention,
%for the convenience of development.
On x86 (64-bit mode), which is the primary target architecture of \graphene{},
...
regulates that the arguments to a function are passed in certain general-purpose registers---specifically stored in \code{rdi}, \code{rsi}, \code{rdx}, \code{rcx}, \code{r8}, and \code{r9}, in order.
Also, by compiling the library OS into an ELF binary,
\graphene{} can simplify the dynamic linking between the application, library OS and PAL,
and enable C compiler optimizations for linking,
