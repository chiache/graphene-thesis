...
\issuedone{1.2.a}{discuss the memory management in the host (threading)}
...
\graphene{} adds two functions to the host ABI: \palcall{RpcSendHandle} and \palcall{RpcRecvHandle}, to send a handle over a connected RPC stream.
The design of \palcall{RpcSendHandle} and \palcall{RpcRecvHandle} is motivated by the Linux file descriptor passing feature of UNIX domain sockets.
...
is a physical memory store;
during process migration, the parent process
can send application memory to the physical memory store, which will keep a snapshot of the sent memory, using copy-on-write.
The child process can then attach to the physical memory store,
and map the memory snapshot into its memory, also backed by copy-on-write.
