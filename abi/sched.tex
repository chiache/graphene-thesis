...
\palcall{MutexCreate} creates a mutex (mutual-exclusive) lock for atomic execution of a critical section in the code.
...
A mutex lock can be acquired by \palcall{ObjectsWaitAny} (described later),
...
\palcall{SynchronizationEventCreate} and \palcall{NotificationEventCreate} create a waitable event for synchronizing the execution of multiple threads.
These two \hostapis{} create two types of waitable events, namely synchronization event and notification event.
Both types of events can be waited by a thread. %, using \palcall{ObjectsWaitAny}.
The difference is that
setting a synchronization event
wakes up at most one thread that are currently waiting for the event,
whereas setting a notification event stops any thread from
further blocking on the event.
An event can be set by \palcall{EventSet}, and be waited by \palcall{ObjectsWaitAny}.


Synchronization events and notification events are designed for different purposes.
A synchronization event can be used in a producer-consumer model, wherein a consumer thread can block until a producer thread submits a task or a result.
...
\begin{paldef}
HANDLE ObjectsWait    (HANDLE *handles, u8 nhandles,
                       u64 timeout);
\end{paldef}
