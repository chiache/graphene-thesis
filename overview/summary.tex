...
%In \graphene{}, 
\graphene{} defines a {\em host ABI}, as a new boundary between the OS and user space.
The host ABI is designed to be simple enough to port on a new host (containing \palcalls{} functions),
but expose sufficient functionality from the host to run the virtualized OS components, as a {\em library OS}.
%The library OS
%implements \graphenesyscalls{} Linux system calls, to reuse unmodified Linux applications.
The host ABI disconnects the complexity of reproducing ``legacy'' system interfaces for reusing applications,
