\section{The host interface}
\label{sec:overview:host}

\issuedone{1.1.b}{Describe \thehostabi{} specification}
\graphene{} starts with defining a simple but sufficient host interface;
the host interface
contains OS abstractions essential to application execution
...
Defining such a host interface separates
...
Upon the host interface,
\graphene{} moves OS components
such as the system call table
and namespaces into the guest space as a \libos{}.
In short,
for porting \graphene{} to any new host OS or hardware,
OS developers only have to focus on
porting a simple host interface
%to new host OSes and hardware,
instead of reproducing the whole API for backward compatibility.
...
Windows also has \syscall{ReadFile} and \syscall{WriteFile}
...
\subsection{\hostapi{} definitions}
...
\subsection{The PALs (Platform Adaption Layers)}
...
For instance, the Windows API disallows directly resizing or protecting part of a memory region, which is essential to implementing the \syscall{munmap} and \syscall{mprotect} system calls.
