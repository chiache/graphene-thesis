...
\Thehostabi{}
contains only OS abstractions that are essential to application execution.
...
the implementation of system APIs (application programming interfaces), which determines the compatibility against applications,
from hardware management components, such as file systems, network stacks, and device drivers. 
\graphene{} moves API components
into a \libos{} and reimplements the functionality using the host ABI.
To enable \graphene{} on a new host OS or hardware,
OS developers only port the host ABI to the target host system interface,
%to new host OSes and hardware,
instead of paying the tremendous cost of rebuilding the whole Linux system call table, as illustrated in Figure~\ref{fig:overview:porting}.
...
For each host system interface, \graphene{} uses
a thin library to export the host ABI,
which is called a {\bf platform translation layer (PAL)}.
%is loaded below the library OS, to translate each functions in \thehostabi{} to native system interfaces.
The main purpose of adding a PAL is to mitigate the semantic gap
between the host ABI and
a native host system interface.
...
By porting a PAL to new host OSes or hardware,
users can reuse
the same \libos{} implementation to achieve compatibility
against unmodified Linux applications.
To keep the porting effort manageable,
...
\graphene{} currently includes several PALs for popular OSes,
such as Linux, \win{}, \osx{}, FreeBSD.
%and \sgx{} with an untrusted Linux kernel.
Most OSes export a POSIX-like system API which happens to be similar
to most of the host ABI of \graphene{}
in semantics.
The similarity of semantics simplifies PAL development
on these OSes
since a PAL is likely to translate
most of the host ABI
to similar host system interfaces with little efforts.



A few exceptions in the host ABI may cause porting challenges on certain host OSes.
...
but requires running the \win{} PAL with root permissions.
These porting challenges
tends to be the results of design decisions or assumptions made by OS developers.
...
if a PAL does not implement bulk IPC,
a \libos{} can always fall back to RPC-based IPC. 
...
\graphene{} defines {\bf \thehostabi{}}
as the host ABI exported
by each PAL.
\Thehostabi{} contains \palcallnum{} \hostapis{}, which provide a specific set of host abstractions
that are essential for \libos{} development.
...
can be an OS or hypervisor
that contains OS functionality to run a standalone application
or virtual machine.
...
These monolithic OSes
often contain a massive amount of system APIs,
...
\sgx{} applications
cannot trust any OS features except ones that are virtualized by the hardware or completely migrated into enclaves.
The only way to obtain other OS features such as file systems or networking
is to request from an untrusted host OS
through RPC (remote procedure call), which introduces a specific set of security threats~\cite{checkoway13iago,osdi16scone}.
Due to compatibility and security challenges specific to \sgx{},
...
which runs a virtual machine on a hypervisor.
...
Instead, 
\thehostabi{} focuses on higher-level abstractions such as files and network sockets
...
as a virtual machine reuses a whole OS on a virtual hardware interface
whereas a \libos{} runs on a simplified host ABI.
