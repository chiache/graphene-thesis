...
The host ABI 
determines a boundary to partition OS components %, such as system calls and namespaces,
into the library OS,
...
OS components from the idiosyncrasy of hardware. %any potential changes for adopting new hardware.
In general, the isolation happens inside an OS,
by defining a driver interface
to encapsulate the hardware operations.
In Linux or BSD, the virtual filesystem (VFS) defines the generic file operations,
such as \syscall{read} and \syscall{write},
implemented by filesystem drivers and block device drivers.
%On the other hand,
...
The host ABI
tries to define a more ubiquitous system interface above plenty of hosts,
for encapsulating both OS and hardware idiosyncrasy.
...
The host ABI shares several characteristics of a virtual hardware interface running a VM.
A VM can be used as compatibility solution,
relying on
the hypervisor to export a set of generic,
virtual hardware.
%a set of generic, virtual hardware,
%which the VM can control with the same drivers.
Virtual hardware eliminate the development effort for porting a VM,
...
Unlike the virtual hardware, the host ABI does not reuse a whole, unmodified OS.
...
while a VM tries to reuse most code from an OS and relies on existing hardware interfaces,
a library OS
...
Table~\ref{tab:overview:abi} lists \palcalls{} OS functions defined in the host ABI of \graphene{}.
