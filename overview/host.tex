...
The host ABI decouples the low-level OS functionality, such as abstracting hardware resources and operations, from reproducing the idiosyncrasy of system interfaces for applications. 
%OS developers only have to port the host ABI for an OS which needs a boost of compatibility.
\graphene{} moves the implementation of system interfaces
into the user-space, para-virtualized library OS,
leaving only the host ABI to interact with the host OS and hardware.
...
The porting of the host ABI is specific to each host. We define a {\em host} as an OS or a hypervisor which exports a consistent interface to applications or virtual machines.
...
Especially, the Linux-SGX port of the host ABI (also called \graphenesgx{}) faces several unique challenges,
...
We can find lookalike functions in many host OSes, such as \syscall{read} in Linux, BSD, and POSIX, and \syscall{ReadFile} in Windows (for regular files).
...
\paragraph{Checkpoint and migration.}




\subsection{Functions in the host ABI}
...
The functions reflect the common functionality of most OSes, including Linux and Windows.
...
Table~\ref{tab:overview:abi} lists \palcalls{} functions defined in the host ABI of \graphene{}.
...
Only 8 functions are extended by \graphene{}, in order to implement Linux-specific features.
...
Linux also requires 
the x86-specific segment registers (i.e., \code{FS}/\code{GS} registers)
...
\centering
\input{abi-table}
\caption{Overview of the functions in the host ABI of \graphene{}. The ones marked with the symbol $\dagger$ are introduced in the initial publication of \graphene{}~\cite{tsai14graphene} or later extended for this thesis. The rest are inherited from \drawbridge{}~\cite{porter11drawbridge}.}
...
Based on the simplicity of the host ABI, the PAL development is supposed to be straightforward and easy for average developers, except corner cases on a few eccentric hosts, such as Intel SGX.
...
several ``classic'' hosts, including Windows, Linux, OSX, FreeBSD, and Linux with Intel SGX.
