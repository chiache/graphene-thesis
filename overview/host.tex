...
\fixme{the beginning doesn't capture the whole paragraph.}
...
\fixme{give more of the background}
...
To simplify isolation, the functions to access each host abstraction must appear \fixme{clarify ``stateless''} stateless to the host, except for the handles to identify the resources. Each call to the host functions is independent. The arguments given for each call must be always be absolute values, instead of relative values.
