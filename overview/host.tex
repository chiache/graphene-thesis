...
The \graphene{} approach to achieving compatibility is to single out a set of basic OS functions generally needed in applications,
and define as a host ABI (application binary interface).
The host ABI decouples the OS functionality for utilizing typical hardware resources,
such as CPU, memory, and I/O devices,
from the idiosyncrasy of reproducing the system interfaces of an OS standard, such as Linux. 
For OS developers, the host ABI is the only component to customize for each OS or hardware option which needs a boost of compatibility. 
The host ABI virtualizes the execution of the library OS,
which is implemented specifically for the host ABI, similar to a fully paravirtualized, custom-made guest OS.
The host ABI makes the development of the library OS, which absorbs most of the complexity for building a compatible system interface,
as an one-time effort which other OS developers do not have to repeat. 
Thus, the design of the host ABI is meant to reduce the OS developers' burden as much as possible, under the circumstance of not sacrificing too much functionality or performance.
 

The development of the host ABI is specific to each ``host'', which can be defined as an OS or hypervisor with a consistent interface to applications or virtual machines (definition used in the whole thesis).
A monolithic OS kernel, such as the Linux, BSD, or Windows kernel, can be treated as a host, upon all the physical machines on which the kernel can consistently run applications.
...
the combination of the OS and hardware is considered a specialized host to the library OS.
...
%Although the number of functions does not necessarily indicate complexity, removing redundant functions is always beneficial for simplicity.
...
to common OS functions. For example, the \syscall{FileRead} and \syscall{FileWrite} functions in the host ABIs are defined with little difference to the POSIX functions, \syscall{pread} and \syscall{pwrite}, except nuanced details like the order of parameters.
