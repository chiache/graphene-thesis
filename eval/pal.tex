...
The comparison is between (1) \syscall{recv} and \syscall{send} on Linux; (2) \palcall{StreamRead} and \palcall{StreamWrite} on a Linux PAL, with and without a SECCOMP filter ({\bf +SC}) and reference monitor ({\bf +RM}); (3) \palcall{StreamRead} and \palcall{StreamWrite} on SGX PAL, without protection.}
...
The comparison is between (1) \syscall{read} and \syscall{write} over a pipe or an AF\_UNIX socket on Linux; (2) \palcall{StreamRead} and \palcall{StreamWrite} on the Linux PAL, with and without a SECCOMP filter ({\bf +SC}) and reference monitor ({\bf +RM}); (3) the SGX PAL, with and without data protection ({\bf +CHK}).}
...
Lower is better.
The comparison is between (1) \syscall{clone} and \syscall{select} on Linux; (2) \palcall{ThreadCreate} and \palcall{ObjectsWaitAny} on the Linux PAL, with and without a SECCOMP filter ({\bf +SC}) and reference monitor ({\bf +RM}); (3) the SGX PAL.}
...
\caption{The latency of (a) signal an event and (b) competing a mutex among N threads (N is 1 to 8).
Lower is better.
The comparison is between (1) pthread condition variables and mutexes on Linux; (2) Notification events and mutexes on the Linux PAL, with and without a SECCOMP filter ({\bf +SC}) and reference monitor ({\bf +RM}); (3) the SGX PAL.}
...
\parbox{0.59\textwidth}{\centering\bf (a) process creation}
\parbox{0.39\textwidth}{\centering\bf (b) SGX enclave creation}
\caption{Latency of allocating and deallocating memory mappings, using either \syscall{mmap} and \syscall{munmap} on Linux, or \palcall{VirtMemAlloc} and \palcall{VritMemFree} on the Linux and SGX PALs.
(a) shows the combined latency of allocation and deallocation, whereas (b) includes the latency of zeroing memory pages.
%which triggers the host kernel to populate the physical pages. 
The comparison is between (1) \linuxapis{} in a native Linux process; (2) \hostapis{} on a Linux PAL, with the options of enabling the SECCOMP filter ({\bf +SC}) and reference monitor ({\bf +RM}); (3) \hostapis{} in an enclave. No shielding mechanism is needed.}
