...
A system interface can contain early specification of OS functionality and application programming interface (API).
Because modifying a system interface undermines compatibility, OS developers
limit their efforts to the OS internals,
and miss opportunities to make greater improvements
to both OS and application sides.
For example, Linux and similar OSes
introduce \linuxapis{} such as \syscall{openat} as an unracy version
of \syscall{open}, but struggle to completely replace the latter due to the difficulty of modifying all applications.
Generally, deprecating a \linuxapi{} is a length process, wherein users are repeatedly warned
until the process finally expires.
Moreover, a large-scale redefinition of the system interface
will disrupt compatibility even further.
An example is the cancellation of an early version of Windows Vista, 
codenamed ``Longhorn'',
which introduced a brand-new user interface API
and database-like file system, at the cost of compatibility~\cite{spolsky04microsoft-api-war}.
