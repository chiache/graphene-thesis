...
Applications rely on operating systems for abstracting the hardware in OS services.
An OS exports a set of {\em system interfaces},
which encapsulates the control of hardware abstractions, as a convenient programming standpoint for application developers~\cite{ritchie74unix}. 
...
The system interfaces frees the average programmers from getting involved with the intimate details of hardware management~\cite[\S1]{tanenbaum19os-textbook}.
...
For example, the basic system interfaces in UNIX, such as \code{read} and \code{write}, conceal the physical aspects of file storage,
as well the differences between ordinary files and I/O devices~\cite{ritchie78unix-retro}.
...
For an application developer, either \code{read} or \code{write} works invariably on any opened file descriptors,
whereas the triggered hardware operations can vary from accessing storage blocks to processing I/O streams~\cite{ritchie74unix}.
...
Programming with the system interfaces facilitates the reuse of applications
across different physical machines.
The top-down view of a modern OS is an extended machine or a ``virtual machine''~\cite[\S4]{dhamdhere2007os-textbook},
which hides the low-level distinction of the hardware.
The benefit is that an application developer only sees an extended machine with consistent OS functions when programming.
%instead of the distinction of hardware.
An early design point of UNIX is to move a large amount of device drivers
into the OS space,
so that a user program can be small enough to fit into
removable disks and be more  distributable~\cite{ritchie74unix}.
POSIX later doubles down on defining a system interface standard that covers all the typical hardware operations, and thus making the applications machine-independent (i.e., {\em portable})~\cite{ieee-posix}.
...
In most of the modern OSes, a developer can easily write a C program that prints ``hello world'' on any physical machines, in merely five lines of code.



The definition of system interfaces is a ``legacy'' of an OS,
and impacts the reproducibility of existing application artifacts,
or the so-called {\em compatibility} for applications.
A typical commodity OS, such as Linux or Windows, contains
at least thousands of ``legacy'' applications formerly developed and distributed for the users.
Preserving the compatibility for applications prevents these pre-existing, well-adopted legacy applications from becoming obsolete on the newer generations of the OS.
Overall, compatibility is like a ``contract'' between the OS and the application developers,
and it relies on the OS to faithfully implement every necessary system interfaces for an application.
Application developers can trust a {\em compatible} OS to reproduce consistent behavior for an application,
as they previously programmed and tested on a version of the OS.
%will behave consistently on the successors of the OS.


There is a tendency that compatibility has increasingly become an issue in OS development, due to the growing variety among applications and among physical machines.
...
The early UNIX systems only have to deal with a relatively small amount of devices, and applications with very specific purposes such as a command-line shell,
and thus can tailor the system interfaces for the easiness of implementation~\cite{ritchie78unix-retro}.
In modern time, any commercial-class OS has to contain at least thousands of device drivers~\cite{padioleau2008linux-device-drivers}, to comply to the diversity of hardware options.
The system interfaces have also grown way more complex than the original UNIX interfaces, and have to offer more programmable options for new hardware support or
for application convenience~\cite{linux-man-syscall,win-api,ieee-posix}.
The latest sizes of Linux and Windows have both grown to the scale of multiple million-lines-of-code
...
For a well-adopted architecture such as x86,
the Linux developers cannot update or retire a system call
without going through a decades-long process of confirming that no application uses this system call.
...
Primarily, the difficulty to update a substantial part of an OS
...
A commodity OS, such as Linux or Windows, normally contains a large amount of system interfaces, with signficiant complexity.
The Linux kernel defines more than 300 system calls, and each system call
can further define lots of options, sub-interfaces (e.g., opcodes for \syscall{ioctl}), and corner cases that have to be handled.
Similarly, in Windows 8.1, the core system libraries (e.g., \code{nt.dll}) provide more than four thousand APIs.
%, which includes the APIs exported by the core system libraries, contains more than four thousands interfaces;
Even for a POSIX-compliant OS, such as AIX, the POSIX standard contains up to 1,191 APIs (version 1003.1). % including the extensible \syscall{ioctl}.
...
This thesis establishes an OS design and a general design principle, for simplifying the implementation of compatibility in an OS, in case the OS developers intend to adopt a new hardware platform.
The key to reducing the difficulty of implementing compatibility
is to largely reduce the amount of system interfaces that have to be preserved,
when updating the OS implementation.
This thesis shows that the OS developers need not to preserve the complete set of existing system interfaces, when the goal is to preserve the compatibility for a certain amount of applications, on an unconventional hardware platform.



For the OS design, this thesis presents a narrowed set of system interfaces,
or {\em host ABIs},
to be implemented for each hardware platform.
The host ABIs are defined to be completely generic to the hardware platforms,
and contain abstractions that are easy to implement in most existing OSes that require compatibility improvement.
Above the host ABIs, this thesis proposes using a {\em library OS},
a compatibility layer which implement a set of {\em legacy} system interfaces,
from a commodity OS such as Linux, in order to reuse the targetted applications.
...
This thesis shows that a library OS can implement a substabtial amount of OS features that the targetted applications require, 
using the host ABIs, which an OS developer can easily implement, for a new hardware platform.
...
This thesis presents an embodiment of the compatibility-favored OS design, as the \graphene{} library OS.
...
\graphene{} implements \fixme{try to grow this number} \graphenesyscalls{} out of \linuxsyscalls{} system calls,
using \palcalls{} host ABIs. 
%nherited from a precursor \libos{} called \drawbridge{}~\cite{porter11drawbridge}.
The host ABIs contain the most OS features, including threading, memory management, file systems, and network connections.
...
and research OS prototypes (e.g., Barrelfish, the L4 microkernel).
The implementation of the \graphene{} library OS and the host ABIs demonstrates the {\em simplification} and {\em sufficiency} of the proposed OS design, to implement compatibility.
...
for implementing compatibility, either in a commodity OS, or the \graphene{} library OS.
...
This thesis have three primary contributions. First, the thesis presents a OS design, as the \graphene{} library OS, which simplifies the implementation of compatibility, using a narrowed set of host ABIs, and a library OS implemented to reuse unmodified Linux applications.
...
Intel SGX provide the hardware support for trustworthy, isolated execution, as part of the sixth-generation Intel CPU extensions.
Using Intel SGX, application developers can isolate and protect a piece of verified, application code from being attacked by the potentially compromised OSes, hypervisors, and other applications.
The isolated environment created by SGX to run an application is called an ``enclave''. The security benefit of using SGX is to utilize the hardware resources of an untrusted host,
such as a public cloud for rent, or an unsecured machine owned by the customers.
...
In order to program against such a hardware, the common expectation is that the application developers must modify an application, to isolate the application code, partially or wholely, inside an enclave.
If the users intend to run an unmodified application, SGX imposes several restrictions to running ``legacy code'' inside an enclave.
...
\code{cpuid} is forbidden in case that SGX is combined with the hardware virtualization (VT) of the Intel CPUs, because a mode of VT can trap the \code{cpuid} instructions to the hypervisor.
%, which is untrusted in the threat model of SGX.
\code{rdtsc} is also forbidden because the Intel CPUs cannot completely rule out the case that the Time Stamp Counter (TSC) is physically slowed down. 
...
However, the most important restriction imposed by SGX is
to disallow the access to OS functionality, 
because the system interfaces defined by the OS are not trusted in the threat model of SGX.
...
and using the official SDK, the supporting library, or \code{libsgx}, can only provide minimal amount of APIs, which are irrelevant to OS-managed hardware resources, 
...
Theoretically, an unmodified application can be supported on SGX, using a thin redirection layer, which forwards the system interfaces to the OS.
However, the problem is that the application must check the results of system interfaces, since the OS is not trusted to faithfully provide the requested OS features.
The application can perform some simple check against relatively naive attacks
from the system interfaces, such as returning a invalid pointer which may corrupt other memory regions or cause the application to leak sensitive information.
However, for the rest of system interfaces, the required checks against the untrusted OS are more subtle, as the application has to predict the correct results which it can check against. Predicting the results for most system interfaces is likely to be as expensive as implementing the system interfaces.


\subsection{\syscall{brk} and \syscall{mmap}}
