...
A UNIX-style OS~\cite{ritchie74unix} encapsulates hardware resources and abstractions under a {\bf system interface}.
%which is programmable in an application.
The system interface presents a consistent view of OS and hardware features, or a virtualized, extended ``machine''~\cite{dhamdhere2007os-textbook} which programmers see when developing an application.
%compared with the physical machines for running the OS.
A primary benefit for the system interface is to program an application with {\bf portability}, as being independent from any encapsulated hardware options, such as CPU models, memories, and I/O devices. 
...
has been an important factor in modern OS standards,
including Linux~\cite{linux-standard-base}, Windows~\cite{win-api}, BSD~\cite{quarterman85bsd}, 
...
A prerequisite to keeping a portable application usable is system interface compatibility against the application.
{\bf Compatibility} represents the ability of an OS to faithfully reproduce the features of a system interface, as previously programmed in an application.
Maintaining compatibility is important for reusing a portable application
in an OS extended for new hardware.
