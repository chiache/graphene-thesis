...
The development of \graphene{} is an exercise to
renegotiate the trade-off
between compatibility against applications and adaptability against hosts.
According to
a study of the Linux system API usage~\cite{tsai16apistudy},
not every Linux \linuxapi{} are equally important to reusing the applications on different host OSes.
Furthermore, not every applications need to be reused
on an innovative hardware or an alternative system interface.
The Linux system API study shows that
a large amount of the Linux \linuxapis{} are defined for system administration,
such as configuring the ethernet cards and rebooting the machine.
It also appears that these Linux \linuxapis{} are only used by a few administrative tools (e.g., \code{ifconfig}, \code{reboot}),
and it is likely that the host OSes will already contain similar programs.
...
as a {\em shielding layer} for protecting unmodified Windows application against malicious inputs from the OS services.
...
Established upon previous work, \graphene{} holds the following key contributions:
\begin{compactenum}
