...
The main concept of a UNIX-style OS~\cite{ritchie74unix} is to encapsulate hardware resources and abstractions, using a {\bf system interface} programmable by applications
(e.g., \linuxapis{}).
%which is programmable in an application.
A system interface can isolate an application from the idiosyncrasy of hardware interfaces, and thus make the application {\bf portable} across hardware platforms.
To application developers,
a system interface reveals the OS as an extended machine~\cite{dhamdhere2007os-textbook},
which presents a consistent view of all the OS and hardware features
to program with.
...
Many modern OS standards,
including Linux~\cite{linux-standard-base}, Windows~\cite{win-api}, and POSIX~\cite{ieee-posix},
have defined their system interfaces with a similar incentive,
as facilitating application programming and portability.
...
%However, defining a system interface does not guarantee long-term portability of an application.
To continuously utilize a portable application, 
{\bf compatibility} has increasingly become a major prerequisite and challenge
in OS development.
An OS with compatibility
\fixme{Check this: defining compatibility}
owns the ability of reproducing features and semantics of a system interface
for reusing deployed applications.
Due to the rapid growth of applications, the cost for compromising compatibility
has become unacceptably expensive;
an OS like Linux or Windows cannot afford reprogramming
up to thousands or millions of applications,
if the system interface is no longer compatible.
Specifically,
both Linux and Windows have gone to great lengths
to avoid jeopardizing compatibility.
...
but eventually dialed back the changes due to compatibility problems\fixme{find this citation}~\cite{win-vista}.
...
OS developers recognize loss of compatibility to be a problem with severe consequences,
and a direct impact on application usability.
...
In general, OS developers encounter compatibility issues when adopting an innovative hardware or OS design,
which leaks out of the typical abstractions
encapsulated by a system interface.
Hardware developers or researchers
create unconventional hardware designs or features,
to explore opportunities
of revolutionizing system qualities.
A cutting-edge hardware invention may revert or change the assumptions which other hardware are conventionally designed with,
forcing extensive changes inside the OS implementation.




%In extreme cases, an innovative hardware can make unusual design decisions,
%changing OS assumptions
%which are critical to application compatibility.
%%thus affecting the OS's ability to stay
%%compatible against applications.
%The kind of hardware is usually
%for realizing a revolutionary feature, or researching new opportunities in OS or hardware development.
%are cutting-edge technologies, or research-purpose hardware.
%Researchers create
%these hardware
...
A running example in this thesis, as a revolutionary hardware feature,
is {\bf SGX (software guard extensions)}~\cite{intelsgx}.
SGX is an Intel CPU feature, which provides the building blocks for developing
an application capable of self-protection against malicious OSes, hypervisors, and administrators.
...
especially without inter-connected, coherent memory~\cite{gschwind2007,cascaval2002evaluation}. 
...
A challenging problem in OS development
is to prevent compatibility issues from distracting hardware innovations.



\issuedone{1.1.b}{Specify the problem as reducing the gap of interface idiosyncrasy}
The key challenge to maintaining OS compatibility
