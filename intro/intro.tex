...
to make applications more adaptable and distributable.
...
is that the system interface must remain consistent, for compatibility against previously-developed applications.
As hardware development will continues after applications are released,
OS developers are responsible for keeping the system interface compatible against existing applications,
regardless of challenges to developing OS support for new hardware.
...
Compatibility issues emerge when a new hardware design leaks out of the encapsulation of hardware abstractions, while changing the system interface can disrupt the usability or portability of existing applications.
...
A running example in this thesis %of a disruptive hardware design
is SGX (software guard extensions)~\cite{intelsgx},
a new Intel CPU feature which assists applications to protect themselves from malicious OSes and administrators,
...
% in unfriendly for OS developers to make a system-wide change transparent to applications.
...
The problem for preserving the compatibility against applications is to reproduce
the idiosyncrasy of a system interface,
which is defined with certain hardware assumptions.
...
Sometimes, an OS prototype is built for experimenting research ideas~\cite{baumann09barrelfish,unikernels,zeldovich+histar,libra,peter14arrakis,belay14ix},
