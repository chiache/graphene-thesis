...
one can simply use the \syscall{read} and \syscall{write} interfaces
...
Specifically, many OSes offer the convenience of developing applications
...
Application developers need to trust the OS to reproduce consistent behaviors within each executions of an application,
...
Compatibility is also a burden to OS innovations, especially in OS research prototypes
...
each of which 
can further define several options, sub-interfaces (e.g., opcodes for \syscall{ioctl}), and corner cases~\cite{linux-man-syscall}.
%, which includes the APIs exported by the core system libraries, contains more than four thousands interfaces;
...
the supporting library of the official SDK~\cite{intel-sgx-sdk}, \code{libsgx}, contains only the minimum amount of APIs, equivalent to the subset of POSIX APIs which is completely unrelated to hardware management, 
...
whenever it calls a system interface. %, such as a POSIX API or a Linux system call.
...
Theoretically, a thin redirection layer can offer the rest of POSIX API for an application running in an enclave,
...
One characteristic of a UNIX-style program is the opportunity of utilizing multiple processes, and cloning processes using \syscall{fork}.
\syscall{fork} is originally designed for executing commands in a shell environment:
...
Process creation using \syscall{fork} offers the convenience of programming multiple commands as individual programs,
...
\end{lstlisting}
\end{minipage}
\caption{Sample code for Linux applications using inter-process communication.}
\label{fig:overview:proc-example}
\end{figure*}


%Furthermore,
Besides \syscall{fork},
Linux multi-process applications also heavily rely on
inter-process communication (IPC).
%which is challenging to implement on an OS prototype
%or a non-uniform memory architecture.
The IPC
...
%Intel SGX in the encrypted enclave memory,
...
\section{Enforcing security isolation policies}
