...
encapsulates hardware resources and abstractions,
and exports a {\bf system interface} %(e.g., \linuxapis{})
to applications.
%which is programmable in an application.
A system interface can isolate applications from the idiosyncrasy of hardware interfaces, and allow applications to be widely deployed to different platforms.
For application developers, the benefit of a system interface
is to hide the details of bare hardware; meanwhile, the OS presents a more powerful ``extended machine'',
which is more feature-rich and homogeneous than bare hardware~\cite{dhamdhere2007os-textbook}.
...
for reusing existing applications.
Due to the rapid growth of applications, the price of losing compatibility
can be extremely expensive;
%an OS like Linux or Windows
developers cannot afford constantly reprogramming applications,
whenever the system interface is extended for new hardware. % no longer compatible.
%Specifically,
...
but eventually dialed back the changes due to criticism on compatibility~\fixme{find this citation}~\cite{win-vista}.
...
OS developers encounter compatibility problems when adopting an innovative hardware or OS design
which leaks out of the typical abstractions
encapsulated by a system interface.
%A disruptive hardware design or feature
%can help explore research opportunities of gaining revolutionary results.
