...
A running example in this thesis, as a revolutionary hardware,
is {\bf SGX (software guard extensions)}~\cite{intelsgx},
an Intel CPU feature which allows applications to protect themselves from malicious OSes, hypervisors, and administrators.
...
Other representative examples can be found among asymmetric multiprocessing architectures,
...
To be compatible on a new hardware,
the key challenge in OS development is to bridge the gap between
the system interface and hardware prerequisites.
...
contains a lot of OS-specific, idiosyncratic abstractions and behaviors.
For example, Linux has defined up to 300 \linuxapis{},
with a number of flags, sub-interfaces, and corner cases~\cite{linux-man-syscall}, all of which have to be faithfully reproduced by a fully compatible Linux kernel.
