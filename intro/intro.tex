...
A system interface can be portable across machines with different hardware---from CPU models (besides instruction sets), memories, to I/O devices---to facilitate reuse of applications.
...
As extreme cases, some hardware are designed to change the assumptions which the OS and system interface depends on, and affect compatibility with applications.
...
These extreme cases mostly happen on cutting-edge, or research hardware,
which explore the opportunity of tweaking OS or hardware assumptions.
...
Chapter~\ref{chap:study} uses the metrics of API importance, to study the usage of Linux and POSIX interfaces.
