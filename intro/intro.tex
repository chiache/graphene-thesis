...
\fixme{start from the perspective of app developers}
For application developers, 
an {\em operating system} represents a layer which abstracts the complex, idiosyncratic hardware,
and which provides a comprehensive way to program against hardware resources, such as CPU and memory.
Using the hardware abstractions presented by the OS,
an application developer needs not to learn the low-level logisitic of the hardware,
as well as to reprogram her applicatio for every new hardware platform.
The hardware abstractions are defined by the OS in the form of 
{\em system interfaces}.
In principle, a system inteface describes the generic logic of using a certain type of hardware.
...
For instance, in UNIX operating systems, such as Linux, system interfaces are mostly defined as {\em system calls}:
To split CPU cycles or a memory space, an application can use \syscall{clone} and \syscall{mmap} to allocate task contexts and memory regions,
to request the OS to schedule enough resources to support its execution.
Another example is the POSIX-style file system operations, including 
\syscall{open}, \syscall{read}, and \syscall{write},
which encapsulate the common behaviors of connecting, receiving, and transmiting the data over various types of IO devices, including terminals, hard drives, and network interfaces.
...
The foundation of compatibility is that the operating system faithfully reproduces the definition of system interfaces---a definition that the OS developers and application developers have agreed upon. Existing applications expect the operating system to implement the functionality and behavior consistently, across multiple OS versions (i.e., {\em backward compatibility}).
...
The latest trend in CPU technology is that
...
The compatibility of a commodity operating system is difficult to maintain
for two primary reasons. First, the system interfaces defined by an operating system are objectively enormous and complex.
A Linux kernel above the version of 3.0 exports more than three hundreds system calls;
The Windows 8.1 API set, which includes the APIs exported by the core system libraries, contains more than four thousands interfaces; Even POSIX 1003.1 has defined 1,191 interfaces for operating systems that are designed to be POSIX-compliant.
