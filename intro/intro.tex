...
Unfortunately, in a modern OS, system interfaces are not only for the portability of future applications, but also to remain {\em compatible} with all existing applications.
Due to the rapid growth of applications and hardware devices,
OS development is increasingly burdened with implementing the ``legacy'' features and abstractions from the past generations, and loses the liberty to change or deprecate any system interface that may ruin compatibility.
...
The struggle for keeping the system interface compatible
%Change of the system interface will take a long time before updating all the previous applications, potentially 
potentially causes the adoption of new OS or hardware technologies
to lag behind the development. %for adopting new hardware.
...
For OS or hardware developers, achieving or preserving compatibility is often a distraction to innovations, even though compatibility is critical to system usability.
...
when a new hardware design leaks out of the typical abstractions
...
(2) a library OS with rich Linux functionality, including Linux/UNIX-specific challenges such as multi-process support;
(3) demonstrating the practicality on balancing compatibility with performance and memory footprint;
(4) reducing the attack surface to the host OS which enforces security isolation (more description in Section~\ref{sec:intro:security}).
...
SGX also forbids \code{rdtsc} to rule out physical attacks on the Time Stamp Counter (TSC). 
