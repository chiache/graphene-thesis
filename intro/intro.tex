...
Operating systems provide a comprehensive way for developers to program applications against the hardware.
...
The key functionality of an OS is to manage and abstract the hardware resources, by defining a set of generic {\em system interfaces} for system programming.
Application developers then use these system interfaces
%, either statically or at the run time,
to request for the OS-managed hardware resources.
...
In UNIX operating systems, system interfaces are mostly defined in the form of {\em system calls}.
For example, applications can use \syscall{clone} or \syscall{mmap}
to allocate task contexts or virtual memory areas (VMAs),
to exchange for the CPU or memory resources managed by the OS schedulers.
...
Similarly, different types of IO devices, such as storage devices and network interface controllers (NICs), can hide their idiosyncratic operations behind the POSIX-style file operations, such as \syscall{read} and \syscall{write}.
Defining these system interfaces reduces the learning curve for developers to programming against the hardware, allowing them to focus on more important implementation challenges, such as debugging and optimization.
...
The generic system interfaces also improve the opportunity of reusing an application, regardless of the choice of hardware platforms.
The users of an OS can choose the platform with hardware components at various prices and specifications.
...
For example,
the same application should be able to run on either a single-core Pentium-4 3.4GHz CPU or an 8-core Xeon E5 1.8GHz CPU;
The number or the frequency of cores should look no difference to the application under the abstraction of the OS, 
...
except for the impact on performance.
...
In some occasions, a language runtime, such as a Java virtual machine (JVM), imposes another layer above the OS, to reuse the applications across OSes and architectures. However, a language runtime will still rely on the system interfaces defined by each OS, to abstract the hardware resources, as well as to keep the language runtime itself reusable across hardware platforms.
...
To keep the system interfaces programmable, the OS is obliged to maintain the consistency of system interface, or {\em Compatibility}, to ensure continuous support to applications.
...
Existing applications rely on the OS to faithfully reproduce the behaviors and interfaces implemented by the previous OS versions (i.e., backward compatibility). The definition of system interfaces is essentially a ``contract'' between application developers and the OS.
Application developers expect the system interfaces to stay identical;
an application previously developed and tested on an OS should behave the same as running on other OS versions installed by the users.
...
In a production OS such as Linux or Windows, the OS developers go to great lengths to maintain the compatibility. %, especially the backward-compatibility.
In the fear of breaking the backward compatibility, OS developers are often reluctant to update or deprecate a system interface.
...
An example is the HugeTLB support in Linux; on one hand, Linux provides a new control option, \code{MAP\_HUGETLB}, for \syscall{mmap}, to explicit specify the use of the optimization; on the other hand, Linux transparently converts the regularly allocated, large VMAs, to emulate VMAs allocated with the \code{MAP\_HUGETLB} option.
Linux also adds the emulation of old system interfaces in the GNU standard C library (\code{glibc}).
...
Windows 7 even introduces a compatibility mode to align the system interfaces with applications designed for Windows XP, or ``XP mode'', which internally runs the applications in a virtual machine.
...
An OS implements the compatibility to another OS, in order to adopt a set of popular applications.
...
The primary challenge to implementing the inter-OS compatibility
is to provide the complete support of the enormous and complex system interfaces of a production OS.
A Linux kernel above the version of 3.0 exports more than 300 system calls;
The Windows 8.1 API set, which includes the APIs exported by the core system libraries, contains more than four thousands interfaces;
Even POSIX 1003.1 has defined 1,191 interfaces for operating systems that are designed to be POSIX-compliant.
These system interfaces are not just large in number;
...
This thesis establishes an operating system design and a general principle,
for building up compatibility against existing applications, in an operating system which the OS developers intend to adopt for an emerging hardware platform, as well as a developing OS prototype.


The first part of the thesis proposes using a {\em \libos{}},
as a compatibility layer above the operating systems, to implement the system interfaces that the existing applications imperatively depend on.
...
This thesis shows that a \libos{} can implement the rich OS features that the applications need, using a host interface defined with simplicity and minimal assumptions. This \libos{} design, alone with the host interface, is potentially stackable on various operating systems, to benefit both backward and forward compatibility.


This thesis presents \graphene{}, a \libos{} to support unmodified Linux applications on various hosts. \graphene{} implements a major portion of the Linux system call table 
\fixme{try to grow this number}
(\graphenesyscalls{} out of \linuxsyscalls{} system calls),
using a small host ABI (Application Binary Interface) inherited from a precursor \libos{} called \drawbridge{}~\cite{porter11drawbridge}.
The host ABI of \drawbridge{} defines the OS features commonly provided on most platforms, including threading, memory management, file systems, and network connections.
...
The second half of this thesis shows that the compatibility against the existing applications, in an evolving operating system, is actually measurable using a scientific method.
...
%This thesis have three primary contributions:
\begin{comment}
...
on the platform of Intel Software Guard Extensions (SGX)~\cite{intelsgx}.
Intel SGX is a trusted execution hardware introduced in the sixth-generation Intel CPUs extensions, which isolates and protects a piece of verified code from being attacked by potentially compromised OSes, hypervisors, and other applications.
The isolated environment created by SGX to run a specific piece of code is called an ``enclave''. The security benefit of using SGX is to utilize the hardware resources of an untrusted host,
such as a public cloud for rent, or an unsecured machine owned by the customers.
The application code running on SGX has the end-to-end protection enforced by the CPU,
guaranteeing the memory state of the trusted execution to be signed and encrypted when the code and data leaves the CPU package and is eventually stored in the DRAM or swapped out to the storage.
SGX can also prove the integrity of the protected execution, to a remote, trusted entity, as well as the integrity of the CPU itself.
...
In order to program against such a hardware, the conventional expectation is that the application developers must modify the applications, to isolate a small piece of code inside an enclave.
If the users intend to run an existing application borrowed from a commodity operating system, SGX actually imposes several restrictions to running the ``legacy code'' inside the enclaves.
...
As one of the restrictions, the SGX hardware forbids a few x86 instructions,
including \code{cpuid} and \code{rdtsc}, for the clarity of shielding~\fixme{maybe explain or mention this word upfront} these hardware features.
For example, \code{cpuid} is forbidden because SGX can be combined with the hardware virtualization (VT) of the Intel CPUs, and a mode of VT actually can trap the \code{cpuid} instructions to the hypervisor, which is untrusted in the threat model of SGX.
\code{rdtsc} is also removed from the enclaves because the Intel CPUs cannot completely rule out the case where the attackers slow down the Time Stamp Counter (TSC) physically. 
The application developers would have to remove these instructions from the enclave code, or the supporting framework inside the enclaves can implement certain wrapper to emulate the semantic when the use of these instructions triggers hardware faults.
...
to disallow the use of majority of OS functionality, including system calls and dynamic loading of application binaries.
This restriction is actually motivated by the security, because the operating system is not trusted to provide these OS features benignly and genuinely.
By design, the supporting framework of the official SDK, or a library called \code{libsgx}, only contains features that are irrelevant to hardware resources and doesn't require interaction with the untrusted OS,
such as \funcname{memcpy}, or \funcname{malloc}, which is a simplified version of the real libc \funcname{malloc} and only allocates from static memory regions.
These OS features are vastly insufficient for supporting the existing applications. Most of the applications heavily depend on the operating systems to faithfully implement the existing system call semantics.
...
\section{Security Isolation on the Host ABI}
...
\section{Language Support for Hardware Abstractions}
