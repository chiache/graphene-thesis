...
Application developers rely on operating systems for abstracting the hardware features in OS services.
In a UNIX-style OS,
the OS-level hardware abstractions can be controlled by the application developers, 
using {\em system interfaces} as a convenient programming standpoint~\cite{ritchie74unix}.
The system interfaces simplify the programming in user space, 
by exempting the average application developers from getting involved with the intimate details of hardware programming~\cite[\S1]{tanenbaum19os-textbook}.
...
The basic system interfaces in UNIX, such as \code{read} and \code{write}, conceal the physical aspects of file storage,
...
For an application, \code{read} and \code{write} can work invariably on every file descriptors,
whereas the triggered hardware operations can vary
from accessing storage blocks to processing I/O streams~\cite{ritchie74unix}.
...
The OSes also facilitate the reuse and distribution of applications
...
which hides the low-level information of the bare hardware.
The benefit is that application developers only see an extended machine with consistent OS functions,
instead of the distinction of hardware.
An early design point of UNIX is to move a large amount of device drivers
into the OS space,
to make a user program fit into
removable disks~\cite{ritchie74unix}.
POSIX later doubles down on defining a system interface standard that covers all the typical hardware operations, and makes the applications machine-independent (i.e., {\em portable})~\cite{ieee-posix}.
The similar initiative is taken by other open OS standards, such as BSD and Linux~\cite{quarterman85bsd,linux-standard-base}.
%In the current OS ecosystem,
In a portable OS, a developer can write a C program that prints ``hello world'' on any physical machines, in merely five lines of code.



Historically, prioritizing the convenience of application development has increasingly complicated OS development.
In early UNIX development, system interfaces are reportedly easy to implement~\cite{ritchie78unix-retro}.
As the range of applications and physical machines expands, the cost for an OS to retain portability has become higher.
The current Linux kernel contains thousands of device drivers, contributing to \fixme{find these numbers and cite} xxx\% of the source code, or xxx thousand lines of code.
The number of system interfaces in an OS
has also grown significantly to fit to the needs of applications:
...
even POSIX has raised the number of APIs to 1,191, in version 1003.1~\cite{ieee-posix}.


drifting apart of
application and OS development, which requires not just portability, but {\em compatibility} of the OS. 
...
The compatibility of an OS relies on both genericity of system interfaces and {\em backward compatibility} of the implementation.
%For genericness, the application needs the system interfaces to be independent to the hardware platforms.
For backward compatibility, the application expects the OS to consistently implement all the necessary system interfaces, as previously defined.
...
Compatibility is essentially a ``contract'' between application developers and the OS;
%Application developers expect the system interfaces to stay identical;
a compatible OS ensures that an existing application, which is developed and tested on a certain version of the OS and a hardware platform, will behave identically on the same OS installed on other hardware platforms.
...
For a commodity OS, an OS developer would avoid changing or deprecating a system interface, unless she has taken a decades-long process of confirming that no application uses the system interface.
