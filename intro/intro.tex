...
Operating systems simplify the hardware management in application development.
...
OSes free the average application developers from the burden of programming the interfaces and resources of a physical machine.
With the OS as an extended machine,
application developers can use a set of predefined
%An OS exports a set of 
{\em system interfaces}
%which encapsulates the control of hardware abstractions,
as a convenient programming standpoint against the OS and hardware~\cite{ritchie74unix}. 
...
one can simply use system interfaces
\syscall{read} and \syscall{write} to access a regular file, without knowing the physical aspects of storage (e.g., blocks, tracks, and cylinders),
as well as the I/O interfaces customized by manufacturers~\cite{ritchie78unix-retro}.
%OSes facilitate application development and execution.
%in order to fulfill complicated tasks on computer systems.
Ultimately, the OSes reduce the effort of programming an application
to utilize various types and budgets of hardware.



Specifically, many OSes offer the convenience of developing an application
that is not tied to specific hardware.
In these OSes, system interfaces are {\em portable},
%To be specific, OSes reduce the effort of making an application 
as the developed applications can be reused on different hardware configurations (e.g., different CPU frequency, memory size, or I/O devices).
The OSes hide most of the distinction of physical hardware,
except for the instruction set architectures (ISAs), which are mostly determined by compilers.
...
The UNIX design also helps making a program small enough to fit into
removable disks and be more distributable.
...
reinforces the design by defining a portable system interface standard that covers ``all'' the typical hardware operations~\cite{ieee-posix}.
...
For decades, applications are developed against 
The definition of system interfaces is a ``legacy'' to an OS, and has to be preserved for the reuse of existing application artifacts
...
Preserving the compatibility prevents these applications from becoming obsolete on newer generations of the OS~\cite{mckusick1996design}.
...
Primarily, the difficulty to retaining the complexity of an OS
...
can further define lots of options, sub-interfaces (e.g., opcodes for \syscall{ioctl}), and corner cases that have to be correctly handled.
Similarly, in Windows 8.1, the core system libraries (e.g., \code{nt.dll}) export more than four thousand APIs~\cite{win-api}.
%, which includes the APIs exported by the core system libraries, contains more than four thousands interfaces;
Even for a POSIX-compliant OS, such as AIX, the POSIX standard contains up to 1,191 APIs (version 1003.1)~\cite{ieee-posix}. % including the extensible \syscall{ioctl}.
...
This thesis establishes an OS design and a general design principle, for simplifying the implementation of compatibility in an OS, in case the OS developers intend to extend the OS for new hardware.
...
\graphene{} implements \fixme{try to grow this number} \graphenesyscalls{} out of \linuxsyscalls{} system calls in Linux 4.0,
...
Intel SGX~\cite{intelsgx} provide the hardware support for trustworthy, isolated execution, as part of the sixth-generation Intel CPU extensions.
...
The supporting library of the official SDK~\cite{intel-sgx-sdk}, \code{libsgx}, only contain a minimal amount of APIs, equivalent to the POSIX APIs that are unrelated to hardware, 
...
from the untrusted OS, such as returning an invalid pointer to \syscall{mmap} which points to existing memory regions that can be corrupted or can leak sensitive information~\cite{checkoway13iago}.
