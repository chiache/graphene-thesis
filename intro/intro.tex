...
\keepthis{This thesis defines
the ability of reproducing a system interface that satisfies the hard-coded requirement of applications as {\bf compatibility}.}
...
As an OS evolves, a system interface may contain an outdated specification of OS features and application programming interface (API);
~\polish{OS developers avoid modifying the system interface,
despite of missed opportunities to
more significant improvement by adjusting both OS and application sides.}
...
wherein every applications will have to be addressed specifically.
%until the process finally expires.
%Imaginably, a more large-scale modification of the system interface
%may be catastrophic.
Another well-known example is the cancellation of an early version of Windows Vista, 
codenamed ``Longhorn'',
...
As a result, %for OS developers, 
compatibility
is crucial to the usability of a system, but also an unwelcome distraction
in the early stage of innovation.
...
\keepthis{Sometimes, in order to adopt an innovative hardware in an OS, 
compatibility can pose a challenge for OS developers.}
...
~\polish{A hardware like SGX impacts OS development
require redesigning the system interface, causing compatibility issues.}
...
is to bridge the presumed gap between
