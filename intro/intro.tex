...
reproducing the features and abstractions from the past generations, for the {\em compatibility} against applications.
Previously-deployed applications
are designed with dependency to both features and semantics
described by the system interfaces in an OS.
...
The struggle on keeping the system interface compatible against applications
...
System interfaces are essentially ``contracts'' between an OS and application developers,
to prevent released applications from becoming obsolete after the OS has evolved.
