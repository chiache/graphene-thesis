...
A UNIX-style OS~\cite{ritchie74unix} encapsulates hardware resources and abstractions inside a {\bf system interface}.
A system interface consistently exports a set of programmable OS features, to facilitate applications;
from the perspective of application developers,
a system interface presents
a virtualized, extended machine~\cite{dhamdhere2007os-textbook},
differentiated from
the physical machines which actually run the OS and application.
Programming against a system interface makes an application {\bf portable},
...
Most modern OSes, such as Linux~\cite{linux-standard-base}, Windows, BSD~\cite{quarterman85bsd}, and OSes designed with the POSIX standard~\cite{ieee-posix},
...
A prerequisite to continuously using a portable application
in to keep the system interface {\bf compatible} against the application.
The development of a system interface
must consider future extensions for supporting new hardware.
The compatibility of a system interface
is essentially a responsibility of the OS developers.
Especially,
OS developers can face challenges
when a new hardware leaks out of the abstractions encapsulated by a system interface;
as a result, OS developers may have to extend the system interface, to facilitate the utilization of new hardware.
Extending the system interface ruins
an existing application's opportunity of utilizing the new hardware,
until the application is reprogrammed, or {\em ported}, onto the new system interface.
...
Other motivating examples can be found among asymmetric multiprocessing architectures,
especially architectures without inter-connected, coherent memory~\cite{gschwind2007,cascaval2002evaluation}. 
...
An important challenge to developing a hardware
and the corresponding OS support
is to prevent compatibility issues to become a distraction to innovation.



\issuedone{1.1.b}{Specify the problem as reducing the gap of interface idiosyncrasy}
To develop a compatible OS on a new hardware,
the key challenge is to bridge the gap between
a complex, idiosyncratic system interface, and the hardware prerequisites.
...
Most of the existing system interfaces,
including the Linux \linuxapis{}, 
contains a lot of abstractions and behaviors which are unique to the OSes.
For example, Linux has defined up to 300 \linuxapis{},
with a number of flags, sub-interfaces, and corner cases~\cite{linux-man-syscall},
many of which are not shared with other OSes.
Developing a fully-compatible OS, to reuse any application,
requires faithfully reproducing
the idiosyncratic features of a system interface,
...
regardless of restrictions from hardware.
