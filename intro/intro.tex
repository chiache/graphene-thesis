...
A UNIX-style OS
encapsulates hardware resources and abstractions~\cite{ritchie74unix}, using a programmable system interface exposed to applications.
...
%The encapsulation of system interfaces
%accelerates the advance of both software and hardware technologies, by making them easier to 
%be deployed within the system stack.
A system interface is portable across hardware configurations, including CPU models, memory sizes, and I/O devices, to broadly reuse applications.
...
Besides OS restrictions for an application to be compiled with a choice of CPU architecture (e.g., x86),
a system interface is mostly independent
...
A prerequisite to programming a portable application in an OS
is to have a consistent
system interface which is {\bf compatible} with previously-developed applications.
As hardware development will continue after an application is released, OS developers must maintain the compatibility of system interface,
while developing OS support for new hardware. %, to ensure portability of applications. 
 %, in response to new hardware technologies.
%which is often a distraction to innovations. %, despite being a critical factor to whether an OS or hardware component is immediately deployable. %, even though compatibility is critical to system usability.
...
%The early UNIX system interface is reportedly easy to implement, due to
%they only have a relatively small amount of hardware devices and applications to deal with, and 
%being tailored
%to existing applications and hardware devices~\cite{ritchie78unix-retro}.
%The problems lies in the complexity of
%due to rapid growth of applications and hardware products,
%For decades, the hardware technology has significantly grown.
%OS development nowadays is increasingly burdened by implementing a compatible system interface,
Developing an OS with compatibility is challenging when a new hardware leaks out of the abstractions encapsulated by the system interface.
If an OS developer has to extend the system interface
to facilitate the programming of new hardware, the extension will be difficult to benefit existing applications.
%Changing or extending a system interface can disrupt the usability of existing applications.
%When the current system interface can no longer retain the portability of applications,
%OS developers are forced to
%
%extend the system interface.
%s, OS developers cannot extend the system interfaces
%without ensuring that existing applications will be updated accordingly.
%Compatibility requires preserving the OS functions defined in a system interface,
%which are indispensable to applications.
%If OS developers must extend the system interfaces for a new hardware device,
...
%the extension won't benefit existing applications without application modifications.
%it would almost certainly delay the timing when users can benefit from the new hardware designs.
%The extra effort on compatibility can delay the deployment of an innovative hardware product, or obstruct reusing an application on new environments.
System interface extensions for new hardware are common in most OSes under active development,
usually as opcodes for \syscall{ioctl} in a POSIX-style OS,
or system call options, such as the \code{MAP\_HUGETLB} flags for \syscall{mmap}.
Unless the OS development can be transparent to applications, the adoption of a new hardware will lag behind the invention, due to lack of compatible OS support with existing applications.




As an extreme case, a hardware may change the assumptions which a system interface depends on, causing compatibility issues in many applications.
A classic example %of a disruptive hardware design
is {\bf SGX (software guard extensions)}~\cite{intelsgx},
a new Intel CPU feature which allows applications to protect themselves from malicious OSes and administrators.
...
Another example is an asymmetric multiprocessing architecture,
or an architecture without inter-connected memories~\cite{gschwind2007,cascaval2002evaluation}. 
...
These extreme cases mostly happen on innovative, or research-purpose hardware,
which contribute by exploring opportunities of tweaking the inherited assumptions in OS and hardware development.
...
The definition of a long-lasting system interface, such as Linux system calls or POSIX API, is influenced by the design of plenty pre-existing hardware,
and thus is difficult to change for a disruptive hardware design.
...
or the whole OS. 
A key insight is to reduce the ad-hoc development effort for translating on a specific hardware or OS interface, by reusing a virtualized OS implementation.
...
from both the system interface and the underlying hardware or OS interface.
...
assumes a generic, standard hardware configuration, %including CPU architecture, memory and I/O devices, 
...
As an alternative to a virtual machine, OS developers can partition a OS design,
using a "host interface" with simpler specification and assumption than virtual hardware.

%Although virtualization can help reusing a whole Linux kernel on a OS prototype~\cite{baumann09barrelfish}, but depends on hardware support like VT, and costs lots of memory to run an unmodified kernel~\cite{agarwal15container}.
...
\begin{compactitem}
\item Chapter~\ref{chap:overview} describes the overview of Graphene and design principles.
\item Chapter~\ref{chap:abi} formally defines the host ABI.
\item Chapter~\ref{chap:libos} describes the library OS.
%, which reproduce a rich of Linux features and personality.
\item Chapter~\ref{chap:linux} describes the host ABI on Linux.
%and the enforce of security isolation.
\item Chapter~\ref{chap:sgx} describes the host ABI on SGX.
%to the Intel SGX platform,
%and the shielding of Linux functionality from untrusted hosts.
\item Chapter~\ref{chap:eval} evaluates the performance of \graphene{},
with a security study.
\item Chapter~\ref{chap:metric} evaluates the compatibility of \graphene{}.
%, and evaluate \graphene{} against other ad-hoc translation layers.
\item Chapter~\ref{chap:study} uses metrics of API importance to study Linux and POSIX.
%to study the API usage in Linux and POSIX.
\item Chapter~\ref{chap:related} discusses the related work.
