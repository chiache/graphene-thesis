...
Application developers rely on operating systems for abstracting the hardware features as OS services.
The OS-level hardware abstractions are controllable by the application developers, 
using {\em system interfaces} as a convenient programming standpoint~\cite{ritchie74unix}.
The presence of system interfaces frees the average application developers from getting involved with the intimate details of hardware programming~\cite[\S1]{tanenbaum19os-textbook}.
...
From the perspective of an application developer, either \code{read} or \code{write} works invariably on all file descriptors,
...
The OSes facilitate the reuse and distribution of applications
across different physical machines.
The top-down view of a UNIX-style OS is an extended machine or a ``virtual machine''~\cite[\S4]{dhamdhere2007os-textbook},
...
As a result, in a portable OS, a developer can easily write a C program that prints ``hello world'' on any physical machines, in merely five lines of code.
...
including Linux and Windows~\cite{tanenbaum2006os,kroah2007linux,cusumano1997microsoft}.
Unlike the contemporary OS implementations, the early UNIX systems are reportedly simple and straightforward to implement~\cite{ritchie78unix-retro}.
