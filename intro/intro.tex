...
vulnerabilities.
Unfortunately,
to fully replace the original \syscall{open}, developers need to modify every application; otherwise, Linux developers can never retire
the unsafe version.
...
various complex system APIs on each host.%~\cite{linux-man-syscall}.
...
instructions such as \code{cpuid} and \code{rdtsc}
...
The threat model of \sgx{} distrusts OS services, except APIs that operate entirely inside an enclave (e.g., \syscall{malloc} and \syscall{memcpy}).
...
In summary, the previous porting models of \sgx{} requires modifying application binaries and injecting security checks for all the OS features used by an application.
...
In particular, \syscall{fork} is a unique feature of UNIX-style OSes, such as Linux and BSD,
...
Similar to the complexity of emulation,
...
Chapter~\ref{chap:overview} describes the overview of Graphene %(including the host ABI and the library OS)
