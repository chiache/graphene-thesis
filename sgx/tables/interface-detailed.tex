...
\begin{tabular}{|>{\raggedright\arraybackslash}p{3.8em}|>{\raggedright\arraybackslash\ttfamily}p{6.5em}|>{\raggedright\arraybackslash\ttfamily}p{6.5em}|>{\raggedright\arraybackslash\ttfamily}p{4.5em}|>{\centering}p{3em}|>{\raggedright\arraybackslash}p{14em}|}
\hline
...
\hline
\multirow{3}{3.8em}{File \& directory access}
...
\cline{2-6}
& SOCK\_SEND     & fd,data,size & & safe & \multirow{2}{14em}{Contents secured by TLS/SSL in application level or PAL} \\
\cline{2-5}
...
\hline
\multirow{2}{3.8em}{Stream handles}
& HANDLE\_CLOSE  & fd & & benign & Only as hints to the host \\
\cline{2-6}
& HANDLE\_POLL   & event\_array & polled & DoS & The host may not deliver events \\
\cline{2-6}  
& HANDLE\_SEND   & fd,send\_fds & & safe   & \multirow{2}{14em}{Handle contents and session keys sent over secured RPC} \\
\cline{2-5}
...
\hline
\multirow{2}{3.8em}{Mis\-cel\-la\-ne\-ous}
