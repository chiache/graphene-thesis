...
only specifies the file descriptor and a user buffer to a \syscall{read} or \syscall{write} call.
...
Among \enclavecallnum{} enclave calls defined in \graphenesgx{}, 18 calls are safe from Iago attacks; 8 calls are not security-senstive; 2 calls can be potentially blocked by the host (denial-of-the-service); and only 2 calls that are not yet shielded and left for future work.
