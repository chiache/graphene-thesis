...
To ensure the correctness of dynamic linking,
\graphenesgx{} must trust \thelibos{} and the \glibc{} loader (\code{ld.so}) to protect
the integrity of loading process.
Besides verifying the integrity measurement
of each dynamic library,
\graphenesgx{} can also filter
any environment variables from the untrusted OS
that may change dynamic loading results,
including \code{LD\_PRELOAD} and \code{LD\_LIBRARY\_PATH}.
Finally, for auditing,
\graphenesgx{} can generate a summary of the dynamic linking results,
%extends the remote attestation report generated by hardware with a
%summary of the linking results, 
including the base addresses and Global Offset Tables (GOTs) of all binaries.
This summary can be attached as a signed report with the attestation report signed by the Intel CPUs.
%was linked at runtime.
The thesis leaves the generation of dynamic linking reports as future work.
...
most supported \linuxapis{} are serviced inside the enclave by the \libos{}.
...
as well as an SGX-specific PAL, 
