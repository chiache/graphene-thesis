...
The enclave ABI limits the possible return values to one predictable answer, and thus reduces the space that the OS can explore to find attack vectors to the enclave.
...
Using the same strategy, a TLS/SSL connection can be establish either inside the application or in the PAL,
to secure enclave calls for accessing network and RPC streams,
such as \funcname{SOCK\_SEND}, \funcname{SOCK\_RECV}, and \funcname{SOCK\_SHUTDOWN}.
...
\funcname{YIELD} does not affect the integrity of an application
...
Finally, only two enclave calls, namely \funcname{FILE\_STAT} and \funcname{DIR\_READ}, are {\em unsafe}, because \graphenesgx{} currently do not protect
integrity of file metadata or directory lists.
Checks for these two calls would require signing the file metadata or directory lists
on a trusted host in advance.
Other existing work like Inktag~\cite{inktag} also demonstrate the integrity checks for file metadata.
