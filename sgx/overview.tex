...
It is common for a library to temporarily mark an executable page as writable
...
To support this ELF behavior, we currently map all enclave pages
...
Some ELF binaries contain run-time linking functions (i.e., {\tt IFUNC} functions), which can dynamically determine
the target of symbols.
ASLR (address space layout randomization),
a feature implemented by \thelibos{},
changes the base address of a relocatable binary
in each execution.
...
the integrity of the dynamic linking process.
The shielding code
...
Checkoway and Shacham~\cite{checkoway13iago} demonstrate 
...
including 18 {\em safe} calls (with checkable semantics); 6 {\em benign} calls (no harmful effects); 2 {\em DoS} calls (may cause denial-of-service); and 2 {\em unsafe} calls (potentially attacked by the host).}
