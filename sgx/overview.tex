...
Other \hostapis{}
either are optional as pure optimizations (e.g., bulk IPC),
or do not match with the threat model of \graphenesgx{} (e.g., sandbox creation).
...
including 18 {\em safe} calls (host behavior can be checked); 6 {\em benign} calls (no harmful effects); 2 {\em blockable} calls (may cause denial-of-service); and 2 {\em unsafe} calls (potentially attacked by the host).}
...
Two interfaces are susceptible to liveness issues (labeled {\em Blockable}): \funcname{FUTEX\_WAIT} and \funcname{STREAM\_POLL}.
