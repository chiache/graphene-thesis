...
a file exists, but not enumerate the contents (except by brute force)~\cite{ritchie74unix}.
% querying of all possible file names)
SELinux~\cite{selinux} and other security-hardened Linux variants~\cite{wright+lsm, apparmor},
...
%% SELinux~\cite{selinux} or other security-hardened Linux variants~\cite{wright+lsm, apparmor},
%% may determine search permission based on a number of factors beyond the execute bit, such as a process's role, or the extended attributes of a directory.
...
that indexes in-memory file contents~\cite{bovet05}.
...
%% \item Most of the improvements in Figure~\ref{fig:by-version} remove atomic instructions from the hit path, or at least replace replace a more expensive instruction (atomic increment) with a less expensive instruction (memory barrier).  Advocates of RCU have demonstrated similar results in microbenchmarks~\cite{hart07comparison}.  Yet there are still memory barriers on the fast path, which are an artifact of serializing concurrent {\tt rename} and lookup operations.  \S\ref{sec:update} shows that even these remaining memory barriers can be eliminated with a fairly straightforward change to the rename implementation.
...
%can guarantee that it has no mount points, such as the portals pseudo file system~\cite{freebsd-book}.
...
%% surveys~\cite{bsd-stats}.  The Apple OS X kernel, xnu, is derived from a mixture of 
%% Mach microkernel~\cite{Accetta:1986:MNK} and FreeBSD sources~\cite{osx-book};
%% the file system components are from FreeBSD and have not substantially changed their behavior with
%% respect to directory metadata caching~\cite{lustre-xnu}.  Thus, we focus this discussion 
%% on FreeBSD.
...
%% table for fast lookup within a directory, negative caching, and an alias list for links~\cite{freebsd-book}.
%% A low-level file system may opt out of the default structures if it has a
%% more specialized structure, say for large directories.
%% Because mount points are not visible to the low-level file system,
%% a low-level file system cannot leverage efficient indices for deep hierarchies unless the file system
%% can guarantee that it has no mount points (generally the case only for the portals pseudo file system)~\cite{freebsd-book}.
%% Several previous works have found this restriction onerous, especially in the network file system
%% space~\cite{duchamp94nfs}\fixmedp{Any other cites on nfs?}.
...
%% with a directory cache structure, called the direct name lookup cache (DNLC)~\cite{solarisinternals}.
...
%% Solaris also has a simpler reference mangement system for cached paths than FreeBSD (and more similar to Linux)~\cite{freebsd-complexity}.
...
%% such as FFS~\cite{mckusick84ffs} or ext4~\cite{ext4},
