...
%% \item Most of the improvements in Figure~\ref{fig:by-version} remove atomic instructions from the hit path, or at least replace replace a more expensive instruction (atomic increment) with a less expensive instruction (memory barrier).  Advocates of RCU have demonstrated similar results in microbenchmarks~\citep{hart07comparison}.  Yet there are still memory barriers on the fast path, which are an artifact of serializing concurrent {\tt rename} and lookup operations.  \S\ref{sec:update} shows that even these remaining memory barriers can be eliminated with a fairly straightforward change to the rename implementation.
...
\begin{figure}
\centering
\includegraphics[width=4.5in]{dcache/plots/lookup-breakdown.pdf} \\
\footnotesize
\begin{tabular}{lll}
{\bf Path1:} \tt FFF &
{\bf Path2:} \tt XXX/FFF &
