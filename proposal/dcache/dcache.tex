...
%% and hardware virtualization support~\cite{intel10, nestedpagetables}, 
...
{\em Cuckoo hashing}~\cite{cuckoo04} can be even more efficient. 
\end{comment}
...
\begin{figure}[t!]
\includegraphics[width=1.0\linewidth]{dcache/figures/dcache-data_structure.pdf}
\footnotesize
\caption{Data structures added for fast directory cache lookup. To support {\em fastpath} lookup, we add a 88-byte fast dentry structure to the original \dentry{} and a variable-sized PCC structure into {\tt cred}. 
%% dp: meh
...
%% The merging approach is used by related works like DLFS~\cite{lensing13dlfs},
...
\begin{comment}
\begin{figure}[t!]
\includegraphics[width=1.0\linewidth]{figures/invalidation.pdf}
\vspace{-15pt}
\footnotesize
...
%We use the multilinear~\cite{lemire2013strongly} 2-universal hash function to generate 192-bit signatures,
...
%% such as DLFS~\cite{lensing13dlfs},
...
Our design uses the 2-universal multilinear hash function~\cite{lemire2013strongly}.
...
\begin{figure}[t!]
\centering
\includegraphics[width=.7\linewidth]{plots/hash.pdf}
\vspace{-10pt}
\footnotesize
\caption{
...
using the 2-universal multilinear hash function~\cite{lemire2013strongly}  (mhash)
and, as a PRF,
...
errors~\cite{Debnath:2010:CSU:1855840.1855856, Srinivasan:2012:ILI:2208461.2208485, Quinlan:2002:VNA:645371.651321, Zhu:2008:ADB:1364813.1364831}.
...
%Because mean-time-to-failure (MTTF) of most hard drives ranges from $10^6$ to $1.5\times10^6$ hours~\cite{schroeder07mttf},
...
%, such as SELinux~\cite{selinux} and AppArmor~\cite{apparmor},
as well as how we 
...
module framework (LSMs~\cite{wright+lsm}), upon which SELinux~\cite{selinux}, AppArmor~\cite{apparmor}, and others are built.
...
%% SOSP Space
%% In contrast, the recent DLFS file system~\cite{lensing13dlfs} minimized disk seeks
...
Plan 9's {\em lexical}
path semantics~\cite{pike00lexical}.  %, on an opt-in basis.
...
%Unix behavior\cite{atime-sux}, except when needed by a legacy application.
...
direct lookup on network file systems, such as NFS versions 2 and 3~\cite{sandberg85nfs}.
...
a stateful protocol with callbacks on directory modification, such as AFS~\cite{howard88} or NFS 4.1~\cite{rfc5661}.
...
%with process-local, lexical substitution tables, similar to Plan 9~\cite{pike00lexical}.
