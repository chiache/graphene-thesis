...
%% and fetching metadata of objects.\fixmetsai{references here}~\cite{duchamp94nfs}
...
%% but passes requests down to lower-leveled file systems on a cache miss~\cite{zadok06, skinner93}.

%\fixmedp{F1: Maybe thread together an argument about why no one would have tried a one-hop lookup before?}


%\marginpar{\scriptsize \textcolor{blue}{ Michael, I think the high-order bits are mostly right on Fig~\ref{fig:lookup-frac},
...
%Most filesystem-related utilities can spend up to 70\% of execution time in lookup-related system calls. Especially the access/stat family can spend up to 65\% of execution time.
}
...
%a per-component tree walk can be replaced with a single hash table lookup (\S\ref{sec:dcache}).
...
%all memory barriers from the lookup path (\S\ref{sec:update}).
...
\begin{compactitem}
\item {\bf Minimize the cost of a cache hit.} (\S\ref{sec:dcache}).
...
\item {\bf Maintain legacy compatibility.} (\S\ref{sec:generalize}).  Unix path semantics are complex, required by applications, file systems, and security modules, frustrating otherwise straightforward optimizations.  However tempting it may be to redesign path behavior to facilitate caching, path operations must exhibit the same behavior, with lower latency.
\item {\bf Never miss the same request twice in quick succession.} (\S\ref{sec:readdir}).  A number of less-frequent operations, such as reading a directory or secure temporary file creation, always miss in the cache {\em even if enough information is in cache to satisfy the operation.}  
...
%and that the worst case microbenchmark is only 12\% slower within \fixmedp{XX}\% of unmodified Linux.
...
%with Mac OS X, FreeBSD, and Solaris in \S\ref{sec:background}, and explain inline how each
...
%% \paragraph{Goals and Design Principles.}
%% Because of the performance gap between memory and storage media,
%% we suggest to make \dcache{} aggressively caching
%% any information that can potentially useful in the future.
%% Although an aggressive strategy may increase requirement of memory space,
%% it is our opinion that modern computers often have enough space to
%% keep a high hit rate,
%% \fixmetsai{it will be better if we can find supporting material}
%% and it is up to the replacement algorithm
%% to mitigate the effect of aggressive caching while having memory pressure.

%%The paper is organized as follows. 
%%Section~\ref{sec:background} gives brief background of metadata caching in commodity OSes.
%%Section~\ref{sec:dcache} describe implementation of {\em fastpath} to speed up directory cache lookup.
%%Section~\ref{sec:update} explains the strategy of removing hardware memory barriers for more efficiency in lookup.
%%Section~\ref{sec:readdir} discusses directory listing result caching.
%%Section~\ref{sec:eval} presents experiment results. 
%%Section~\ref{sec:related} discusses related works.
%%Finally, section~\ref{sec:conclusion} presents conclusion of the work.
