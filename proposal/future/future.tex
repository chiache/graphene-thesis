...
The \liboses{} can enforce DAC-type (Discretionary Access Control) policies,
by labeling \picoprocs{} with higher secrecy or integrity.
Among the \picoprocs{}, \term{information flow tracing} can be applied to
determine whether a \picoproc{} is allowed to send or receive RPC messages
on host streams connected with other \picoprocs{}.
The directional access control on host RPC streams is
reasonable extension to the host ABI.
