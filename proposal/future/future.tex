...
can be assigned with \term{credential IDs} or \term{capabilities},
...
SELinux can enforce a \term{multi-level security} model, or \term{DAC (Discretionary Access Control)},
...
In addition,
Linux supports sandboxing of certain \term{namespaces},
...
Supporting the security models is important for \liboses{}:
they are not only designed to restraint unrelated applications,
but also to ensure the safe behaviors of cooperating components.
...
vulnerable to local, in-process attackers. % inside the same \picoproc{}.
...
based on the \term{information flow control} by the kernel.
Applying this model to \graphene{} will require
massive changes to the design;
all multi-process abstractions coordinated over RPC will have to be mediated by the kernel, with inevitable extension of the host interface.
