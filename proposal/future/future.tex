...
\item {\bf September:}\\
Collecting and applying the feedbacks from the committee.
...
Simultaneously, preparing for publication goal~\ref{enum:tocs-graphene}.

\item {\bf October:}\\
Delivering reasonable and significant improvement on
partitioning
\java{} applications an
reducing the TCB in \graphene{} \libos{} (as described in \S\ref{sec:future:partitioning}).
Simultaneously, preparing for publication goal~\ref{enum:eurosys17}.
 
\item {\bf November:}\\
Exploring more generalization of the legacy support and platform independence in \graphene{} \libos{} (as described in \S\ref{sec:future:independence}).
Designing the platform-independent host abstractions
for implementing the missing \libos{} features (Table~\ref{tab:future:abi}).
Delivering and experimenting a total port of \graphene{} \libos{} on OSX, Windows, Barrelfish and L4.

\item {\bf December:}\\
Exploring the feasibility and challenges for achieving non-binary security isolation in \picoprocs{} and enclaves.
Several use cases need to be shown to
demonstrate the practicality of non-binary security isolation.
Based on the preliminary concepts of non-binary security isolation, anlayzing the potential threat in such a model.
...
\item {\bf January:}\\
Delivering a preliminary design of non-binary security isolation for \graphene{} \libos{} in both \picoprocs{} and enclaves.
Proof-of-the-concepts must be provided to demonstrate the prevention of vulnerabilities in the model.
Considering possible research contributions
annd deciding a publication plan.

\item {\bf February:}\\
Finishing a complete draft of doctoral dissertation,
...
\item {\bf March:}\\
Collecting and applying feedbacks based on the suggestions
of the committee.
Continuing polishing the dissertation draft
and preparing for the dissertation defense.

\item {\bf Mid-April:}\\
Defensing doctoral dissertation and submitting the final dissertation paper.
