...
%\fixmedp{This list feels like fluff to me.  Commenting for now, unless you have a good argument to resurrect it}
%\fixmetsai{This list is a trailer. It pictures what kinds of insight we are looking for, so readers would know what they can expect.}
%% dp: Adding a few sentences at end
\begin{comment}
Based on our study, we suggest improvements for the following qualities of Linux APIs:
\begin{compactitem}
\item {\em Cleanness}:
Deprecating unused APIs can keep the interface
implementation clean,
saving the efforts for developers to maintain, test and document
the APIs.
\item {\em Security}:
Shrinking or partitioning APIs can reduce
unnecessary attack surface, preventing unknown vulnerabilities
in these APIs to be exploited.
\item {\em Ease of improving compatibility}:
Knowing the APIs with the highest \usagemetric{}, developers can find
the shortest path to boost the overall system compatibility.
\item {\em Resource Usage}:
As a minor benefit, deprecating APIs can free up system resources occupied, for storing API states, directory entries or symbol tables.
\end{compactitem}
\end{comment}
...
\vspace{-0.1in}
\center{
\includegraphics[width=0.9\linewidth]{syspop/figures/syscall-popularity-all-by-inst.pdf}
}
\caption{The trend of \usagemetric{} as N-most important system calls among total \syscallnum{} system calls of \osversion{} with Linux kernel \kernelversion{}. 
	%(\byinst{}) shows the \usagemetric{} based on total application installations and (\byvote{}) represents \usagemetric{} based on regularly used applications.
...
\begin{figure}[t]
\center{
\includegraphics[width=.95\linewidth]{syspop/figures/syscall-compatibility.pdf}
}
\vspace{-0.1in}
...
\begin{table}[t]
\center{
\begin{tabular}{c>{\footnotesize\raggedright\arraybackslash}m{0.42\linewidth}>{\raggedleft}m{0.08\linewidth}>{\raggedleft\arraybackslash}m{0.2\linewidth}}
\toprule
Stage & {\normalsize Sample System Calls} & \# & {\footnotesize \CompatMetric{}} \\
\addlinespace
\midrule
I &
...
{\tt capset}, {\tt reboot}, {\tt unshare}, {\tt tkill}
& +57 (202) & 90.61 \% \\
\midrule
...
\footnotesize
\caption{Five stages of implementing system calls based on the \usagemetric{} ranking. For each stage, a set of system calls is listed, with the work needed to accomplish (\# of system calls) and the \compatmetric{} that can be reached.}
...
%% %For instance, Graphene library OS~\cite{tsai14graphene} only implements {\tt FIONREAD} of {\tt ioctl} to satisfy the most common usage in applications. 
...
\vspace{-0.1in}
\center{
\includegraphics[width=0.85\linewidth]{syspop/figures/ioctl-popularity-by-inst.pdf}
}
...
\includegraphics[width=0.57\linewidth]{syspop/figures/fcntl-popularity-by-inst.pdf}
\hspace{-0.1\linewidth}
\includegraphics[width=0.56\linewidth]{syspop/figures/prctl-popularity-by-inst.pdf}
}
...
%In our experience building the Graphene library OS~\cite{tsai14graphene}, {\tt gcc}
...
\center{
\small
\begin{tabular}{>{\footnotesize\raggedright\arraybackslash}m{1.9in}>{\raggedright\arraybackslash}m{1.1in}}
\hline
\addlinespace
