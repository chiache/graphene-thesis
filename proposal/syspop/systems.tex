...
\centering
\small
\begin{tabular}{m{0.5in}>{\centering}m{0.25in}>{\raggedright\arraybackslash\footnotesize}m{1.5in}>{\raggedleft\arraybackslash}m{0.4in}}
\toprule
...
User-Mode-Linux~\cite{user-mode-linux}, L4Linux~\cite{hartig97mu}, FreeBSD emulation layer~\cite{freebsd-emu}, and Graphene library OS~\cite{tsai14graphene}.
...
This study also uses \compatmetric{} to evaluate the compatibility of several \libc{} variants --- eglibc~\cite{eglibc}, uClibc~\cite{uclibc}, musl~\cite{musl} and dietlibc~\cite{dietlibc} --- against \glibc{},
