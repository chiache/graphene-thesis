...
%filesystem kernel component are different than other standard applications~\cite{file-not-file}.
%Another study of device drivers classifies drivers into 5 different classes after analyzing the interacion of the drivers with the kernel, devices, and buses~\cite{understand-drivers}.
%SymDrive identifies busgs in device drivers in the kernel by studying I/O changes after the driver patches~\cite{symdrive}. \fixmedp{Is this really related?  I was thinking of a different swift paper}
...
prioritize the implementation effort for new system security policies~\cite{jain14setuid}. % \fixmebj{nice one :)}.
...
The Linux Standard Base (LSB)~\cite{linux-standard-base} 
...
%LSB goal is
%Several previous studies have examined compatibility of different software library implementations~\cite{linux-standard-base}
...
to maintain the compatibility across versions~\cite{shared-library}.
...
type definitions and function signatures~\cite{backward-compatibility}.
...
solutions examine various attributes of the components' source code, such as recursive functions and strong coupling of different classes~\cite{component-compatibility}.
...
%%% Research has been done on the compatibility of components during software development~\cite{component-compatibility}.
%%% There are verifiers for compatibility
%%% between software components such as binaries and libraries based on
%%% broad spectrum of compatibility problems~\cite{backward-compatibility}.
%%% Other studies focus on compatibility of application binaries with the different 
%%% versions of libraries~\cite{shared-library} and with libraries provided in different 
%%% distribution environments~\cite{linux-standard-base}.
...
%% common or stolen code~\cite{software-theft}.
%% \fixmedp{I don't get this one---a little more detail please?}\fixmebj{how about now?}
%% Another work reduces the number of kernel boundary crossings by replacing group of system calls, that are identified by dynamically profiling applications, with a single system call~\cite{multi-call}.
%% %and optimize the performance by clustering multiple related system calls~\cite{multi-call}\fixmedp{or this one?}.
%% Runtime system call tracing of applications has also been used to model 
%% fine-grained power consumption for Smartphones~\cite{power-modelling}.
%% CDE~\cite{cde} use system call information of x86 Linux 
