...
\begin{comment}
When evaluating the thoroughness of a system prototype that has a Unix compatibility layer,
papers often appeal to the number of total supported system APIs
as a noisy indicator that the prototype probably covers all important use cases.
Such a metric is easy to quantify, but problematic.
\end{comment}
...
by Debian and Ubuntu~\cite{ubuntu-popularity,debian-popularity}.
...
%Although difficult to measure directly, we approximate this based on package installation statistics~\cite{ubuntu-popularity}.
...
%% To build the metric, we also import the package popularity statistics from the official \osdist{} reports~\cite{ubuntu-popularity, debian-popularity}.
...
(Figure~\ref{fig:executable-type}).
