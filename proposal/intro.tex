...
or the old ones have to be \term{emulated} in the new OSes.
...
However, a modern architecture can be still too subtle to emulate---the hardware capability for virtualizing a guest OS
can be restricted in certain architecture platforms,
such as previously mentioned SGX enclaves.
...
and, second, building OS emulation that satisfy majority of applications.
...
The multi-process abstractions to emulated by \graphene{} include
...
The \graphene{} \libos{} provides not only free OS emulation on the platforms
where the host interface can be ported,
but a way to estimate and reason about the completeness of emulation
when certain host features are unavailable.
For example, allocation and assignment of thread-local storage based on x86 segment registers ({\tt FS}/{\tt GS})
is required in numerous applications.
We observe that, as the host platform may confiscate the segment registers for its own purpose,
implementing this host interface ({\tt DkSegmentRegisterSet}) can be infeasible on many platforms, such as Win32 or OSX.
Fortunately, \graphene{} allows the host interfaces to be partially ported,
while the developers can determine or even measure the consequence of missing the interfaces---such as acknowledging that applications statically accessing segment registers will be broken.
Moreover, for specific interfaces,
the \libos{} can provide generalizable, platform-independent workarounds to address the portability,
