...
another first-order requirement in systems is
to pursue low latency or high throughput,
according to how users define quality of service.
Nevertheless, there is a trade-off between the emulation of OS personalities
...
In general, due to the complexity of building or preserving complete legacy support,
developers often prioritize the implementation
based on what they believe to be more important
...
collected in the Popularity Contests~\citep{debian-popularity, ubuntu-popularity}.
Based on the study,
we create a fractal measurement,
for estimating the fraction of applications installed by a random user
that can plausibly use the system.
For instance, in the current prototype of \graphene{},
in which we choose Linux system calls by the target applications for experimenting,
...
In general, we explore the possibility of reusing the legacy code for
either
...
The reason for pursuing \java{} application partitioning
is two-folded.
On the one hand, legacy applications developed in \java{} or other managed languages
are supported by
isolated environments such as \intel{} \sgx{} enclaves.
...
with legacy code optimally reused.
