Unknown security vulnerabilities and performance bottlenecks inevitably exist
in modern applications and operating systems,
%The pitfalls hidden in the legacy code affect users
in a way that even developers cannot foresee clairvoyantly.
An established strategy to contain
the consequence of hidden pitfalls %potential vulnerabilities or bottlenecks
is to partition a complex system into
smaller, reasonably simple pieces, with strict isolation.
...
the soundness of the design and implementation in the partitions
...
or \emph{\picoprocs{}},
in order to isolate mutually distrusting applications.
The security isolation enforced by library OSes
is based on the fact that
every \picoprocs{} have only a narrow attack surface to the kernel,
while the library OS instances are isolated by address space.
...
identify and evict the OS logics causing congestion,
such as multiplexing hardware resources or allocating processing buffers,
into applications or library OSes.
As a results, these logics that are unneeded or redundant for some applications
...
A key challenge for a system being largely redesigned or restructured is to
reuse legacy code from prior systems and applications,
in order to harvest the enormous development effort already made in prior systems.
With millions of applications built and deployed
on mainstream OSes like Windows and Linux,
