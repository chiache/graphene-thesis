...
or isolated execution platforms like \term{\intel{} \sgx{} enclaves}.
...
the Linux \term{file system directory cache}.
...
Generally, when adopting a new system design, developers struggle to
implement or preserve legacy support
with prefect backward-compatibility.
The reality is, developers often prioritize the development
based on what they believe to be more \emph{important},
so the decisions become skewed toward the applications they prefer.
We suggest a new approach to guide and evaluate the implementation
of legacy OS abstractions~\citep{tsai16apistudy}.
The approach
uses a representative study
on how system APIs (e.g., system calls in Linux)
are used in applications,
weighted by the application popularity
among the users,
to estimate the fractal completeness of a built system.
According to the study, in a random installation, the applications
supported in the \graphene{} prototype
will be improved from 0.42\% to 21.1\%
by merely adding two scheduling APIs that are not foreseen by the developers.
%On the other hand,
