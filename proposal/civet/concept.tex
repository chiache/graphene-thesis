...
To provide the required OS APIs inside the enclave, we use the Graphene library OS~\cite{tsai14graphene}~\footnote{Downloaded from \url{github.com/oscarlab/graphene}}.
...
%To avoid the cost of porting the \java{} VM, \systemname{} uses {\em Graphene-SGX library OS}~\cite{graphene-sgx} 
...
\systemname{} framework uses a byte code generation library --- {\em CGLib}~\cite{cglib} to create untrusted proxies for the in-enclave instances.
