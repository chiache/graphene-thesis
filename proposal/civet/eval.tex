...
This section evaluates \sysname{}'s memory footprint, startup time, performance, and TCB size.
We drive this evaluation 
%with the case studies (\S\ref{sec:case-study}),
%as well as with
using Java Microbenchmark Harness (JMH) Suite~\citep{jmh}.
...
The first two columns of Table ~\ref{tab:civet:perf} --- Native Linux and Graphene in SGX --- shows the overhead of isolating any application in SGX is about \fixme{XX}\%. Graphene adds an additional \fixme{XX}\% overhead on the memory footprint. The startup time indicates the cost of creating an enclave is \fixme{XX}\%. 
