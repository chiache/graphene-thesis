...
The \systemname{} runtime framework builds on Phosphor~\citep{bell2014phosphor}
...
%\systemname{} inherits the threat model of \sgx{}~\citep{sgx}.
...
%\systemname{} do not defend against attack vectors such as side-channel, covert-channel and control-channel~\citep{control-channel}. Denial of Service (DOS) attack is not part of \systemname's threat model. Even if the OS never schedules the enclave program or the untrusted part of the application is manipulated to never enter the enclave, no provisioned secret is leaked outside the enclave.
