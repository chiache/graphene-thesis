...
\vspace{5pt}
\noindent{\bf Implementing fork by (ab)using checkpoints.~} 
...
\vspace{5pt}
\noindent{\bf Inheriting file handles.~}
...
which are commonly used by sandboxing systems. % such as plash~\cite{plash}.
...
%and  reintroduce security problems caused by accidental inheritance~\cite{close-on-exec}.
The transfer of data is explicit to the host, can be mediated by a reference monitor,
the sender, or the receiver.
For instance, recent Unix systems introduced a close-on-exec flag for file handles~\cite{close-on-exec}, 
