...
Our benchmark shows that applying a seccomp filter can cause \fixme{xxx}\% overhead on {\tt getppid} syscall latency. We use a seccomp optimization patch, which just-in-time compiles filter code into machine code \fixme{reference}.
...
\end{tabular}
\caption[\sysname{}: \libos{} for System V message queues]
...
Table~\ref{tab:graphene:msgq} lists \libos{}s
