...
\vspace{5pt}
\noindent{\bf Seccomp filter optimization.~}
Our benchmark shows that applying a seccomp filter can cause xxx\% overhead on {\tt getppid} syscall latency. We use a seccomp optimization patch, which just-in-time compiles filter code into machine code \fixme{reference}.
...
\hline
\end{tabular}
\caption{Startup, checkpoint, and resume times for a native Linux process,
a KVM virtual machine,
and a Graphene picoprocess, where appropriate. Lower is better.  
Overheads are relative to Linux.} 
\label{table:lmbench2}
\end{table}
...
Graphene supports migration of an application from a picoprocess on one machine
to a picoprocess on another machine by checkpointing the application,
...
Table~\ref{table:lmbench2} shows the time to start
up a process, VM, or picoprocess; as well as the checkpoint and resume time for KVM and Graphene.
...
Similarly, checkpointing and restoring a 4 MB application on Graphene is 1--4 $\times$ faster than checkpointing or restoring a KVM guest.
...
Thus, one would expect roughly 1 MB of extra memory usage for any single-picoprocess application.
...
Figure~\ref{fig:memusage} lists memory overheads of 
...
%\caption{Memory usage of applications on Linux, Graphene, and KVM, in MB.  Lower is better.
%\label{fig:memusage}}
%\end{figure}
...
In these samples, however, Graphene also had 3--4$\times$ as many processes
running; this is because Unixbench simply spawns all of the tasks in the background, rather than
executing them sequentially.   Because Graphene processes execute more slowly (attributable to a slower {\tt fork}---\S\ref{sec:eval:performance}),
a given sample will include more picoprocesses, pushing total memory usage higher.
Thus, we expect that further tuning {\tt fork} performance will lower sampled memory usage.


Across all workloads, Graphene's memory footprint is 3--20 times  
smaller than KVM.  
...
\hline
{\bf \gcc/make} & \multicolumn{2}{|c|}{\bf Linux} & \multicolumn{3}{|c|}{{\bf KVM}} & \multicolumn{3}{|c|}{{\bf Graphene + RM}} \\
\hline
% gcc
...
\hline
{\bf Apache} & \multicolumn{2}{|c|}{\bf Linux} & \multicolumn{3}{|c|}{{\bf KVM}} & \multicolumn{3}{|c|}{{\bf Graphene + RM}} \\
\hline
...
{\bf \light{}} & \multicolumn{2}{|c|}{\bf Linux} & \multicolumn{3}{|c|}{{\bf KVM}} & \multicolumn{3}{|c|}{{\bf Graphene + RM}} \\
\hline
...
%%%%% UnixBench
  ...
  & \multicolumn{8}{|c|}{Execution Time (s), +/- Conf. Interval, \% Overhead} \\
\hline
{\bf bash } & \multicolumn{2}{|c|}{\bf Linux} & \multicolumn{3}{|c|}{{\bf KVM}} & \multicolumn{3}{|c|}{{\bf Graphene + RM}} \\
\hline
%2.1 s linux
%graphene 10.06s
...
\\\hline
\end{tabular}
\caption{Application benchmark execution times in a native Linux process, a process inside a KVM virtual machine, and a Graphene picoprocess with reference monitoring (+RM). }
\label{tab:apps}
\end{table}
...
Table~\ref{tab:apps} lists 
execution time of our {\em unmodified} 
application benchmarks (detailed in \S\ref{sec:eval:mem}).
...
%%% <<<<<<< HEAD
%%% %Apache runs with 4 preforked workers. Lighttpd runs with 4 threads.
%%% =======
%%% Apache uses {\tt Prefork MPM} module, with 4 processes created when server starts. \light{} has no multi-process support but we run it with 4 worker threads. 
%%% >>>>>>> a694f6552d0348c4edc4e6334346550f5b72b239
...
For instance, {\tt make -j4 libLinux} speeds up 3.7$\times$ on Linux and 3.4$\times$ on Graphene.
...
\multicolumn{3}{|c|}{{\bf Graphene + RM
}} \\
...
\end{tabular}
\caption{LMBench comparison among native Linux processes
and Graphene picoprocesses, both without and with the reference monitor (+RM).
...
takes additional work to create a clean picoprocess on Linux; we expect this overhead could be reduced
...
{\tt libLinux} checkpointing code (commensurate with the data in Table~\ref{table:lmbench2}), which 
...
We also measure the overhead of isolating a Graphene picoprocess inside the reference monitor.
...
\end{tabular}
\caption{Microbenchmark comparison for System V message queues
 between a native Linux process and Graphene picoprocesses.
 Execution time is in microseconds, and lower is better.
...
\vspace{5pt}
\noindent{\bf System V IPC.~}
Table~\ref{table:msgq} lists microbenchmarks
which exercise each System V message queue function,
within one picoprocess (in process), across two concurrent picoprocesses (inter process),
and across two non-concurrent picoprocesses (persistent).
...
discussed in \S\ref{sec:namespaces:insights} reduced overheads compared to a naive implementation
...
of Linux pipes, using a microbenchmark that compares the cost of ping-ponging a no-op RPC within a sandbox.
...
The performance of Graphene closely matches Linux (Figure~\ref{fig:rpc48core}),
...
%% %%The RPC overheads indicate that the RPC substrate itself easily scales to 32 picoprocesses
%% %%within one sandbox.  Beyond 32 picoprocesses, the IPC helper threads are not always available 
...
%We validate that the Graphene host can dynamically move a picoprocess into a new sandbox, and that the Graphene library OS handles this transparently to the application. In this test case, the host forcibly disconnects all streams between two guests after execution begins. Both library OSes behave as if the other picoprocess terminated, delivering exit notifications, closing application-visible pipes. The picoprocess which was not already leader for IPC coordination  assumes this role in the new sandbox. Thus, we show that Graphene gracefully handles dynamic sandboxing or disconnection of collaborating picoprocesses.
...
\begin{comment}
In our design, only trusted applications such as system utilities can run as a non-Graphene picoprocess, as they need many more functionality that the Graphene Linux ABI support. From a security standpoint, nothing prevents a Graphene picoprocesses from acting maliciously, as application code runs unmodified in Graphene.  However, it is important to note that Graphene’s goal is not to secure a system but to isolate applications in a competitive level compared to the VM alternative. 
\end{comment}
...
\vspace{5pt}
\noindent{\bf Isolation Experiments.~} This subsection tests whether Graphene meets its goal of providing equivalent isolation
to a VM.
We conducted an evaluation of the security of the isolation mechanisms in Graphene and analyzed whether a malicious Graphene picoprocess could 
...
(iv) discover secrets from picoprocesses in other sandboxes or from non-Graphene process through side-channels via the {\tt /proc} file system. 
...
\begin{comment}
Consider the configuration illustrated in Figure \ref{fig:coordination} and assume that picoprocess 1 is malicious . In our first experiment, picoprocess 1 unsuccessfully attempts to fork a non-Graphene process by invoking the {\tt fork()} function and also invoking the {\tt fork} system call directly. Since Graphene hooks system calls at the kernel level, it does not matter how the fork occurs since the kernel checks whether it is a Graphene picoprocess that is calling {\tt fork} and creates a new Graphene process if it is. 

In our second experiment picoprocess 1 attempts to kill picoprocess 3 in a separate sandbox. Picoprocess 1 is blocked from calling the kill command on any process that is not explicitly contained in its sandbox since the PID is checked against the sandbox's PID list. 
...
In a third experiment picoprocess 1 tries to access files that do not belong in its Manifest. Picoprocess 1 attempts to circumvent the Manifest rules through the function {\tt open}, {\tt fopen}, and also by calling the open system call directly. When the open system call is called, it’s filename parameter is checked against a list of acceptable directories and rejected if it does not match anything in this list. This blocks the picoprocess 1 from accessing illegal files.

In our last experiment, picoprocess 1 attempts to obtain secrets from picoprocess 3, in an attack similar to Memento \citep{memento}. Graphene processes are restricted from using the proc file system in order gain information about system processes.  In our experiment, picoprocess 1's manifest file was setup to deny access to this directory and, as a result, it was unable access system or other processes' information outside its sandbox.
\end{comment}
...
\vspace{5pt}
\noindent {\bf Analysis of Linux Vulnerabilities.~} Graphene restricts picoprocesses to 15\% of the system call table.
...
%\hline
\end{tabular}
\caption{Manual analysis of Linux vulnerabilities from 2011-2013 and Graphene's prevention.}
...
confined in the correspondent picoprocess or sandbox. 
\end{comment}
