...
We selected the Drawbridge ABI~\cite{porter11drawbridge} as a starting point for the Graphene ABI,
...
\noindent Graphene requires  the following additional PAL calls:
\begin{compactenum}
...
\item {\tt DkExceptionReturn} --- Returns from an exception back to the host kernel.
   ...
   %% Alternatively, the Graphene ABI could require exception delivery to include
...
Graphene uses bulk IPC in conjunction with control
...
%%% system call. Compare to the native system calls, reloading the whole Graphene
%%% system causes duplicated works such as parsing manifests and linking libraries.

The Graphene PAL optimizes process creation by forking 
a newly launched picoprocess after initializing all of the Graphene state,
...
%%% This strategy of checkpointing the process manager allows Graphene to quickly 
