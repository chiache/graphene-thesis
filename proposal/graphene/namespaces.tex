\section{Guest Coordination Framework}
...
{\bf namespace management}.  Coordinating picoprocesses need a consistent mapping
...
by issuing a remote procedure call (RPC) to the appropriate picoprocess.
...
When picoprocesses are in the same sandbox, they coordinate
...
When picoprocesses are in separate sandboxes,
...
%Figure~\ref{fig:sandbox} illustrates several sandboxes with picoprocesses
...
%If the connection between the two picoprocesses on the right of the figure
...
\begin{comment}
Within a sandbox, each library OS tracks the PIDs of other picoprocesses.
...
If a picoprocess signals itself, the signal system call simply calls the 
appropriate signal handling function in the application. 
If a picoprocess signals another picoprocess,
...
Section~\ref{sec:namespaces} describes these library OS-internal
...
simply by preventing picoprocesses in different sandboxes from 
...
%in each picoprocess to serve remote messages and receive responses atomically.
...
%%% to specific picoprocesses when practical.  
...
\begin{figure}
\centering
%\includegraphics[width=0.7\linewidth]{figures/coordination.pdf}
...
Figure~\ref{fig:coordination} illustrates two sandboxes with picoprocesses
...
%If the connection between picoprocesses 1 and 2 is severed by subdividing the sandbox,
%the processes will become inaccessible to each other
%and each newly isolated library OS will treat the event as a process termination.
...
%In this example, picoprocesses in different sandboxes have the same PID number, but this does not cause conflict as they are isolated and can only communicate with processes in the same sandbox.
...
\vspace{5pt}
\noindent{\bf System V IPC.~} System V IPC
...
%%% A key which does not have a valid mapping can be assigned to an ID by any picoprocess.

\vspace{5pt}
\noindent{\bf Message Queues.~} In Graphene, the owner of a queue ID is responsible for 
...
%\fixmedp{bill: it is referenced in the Semaphores section, but here there isn't any talk about migrating msg queues to the most active picoprocess. also, the section starts with ``two essential optimizations. First \ldots'', but there isn't a second. is queue ownership migration the second?}
...
%Because we assume that picoprocesses within a sandbox trust each other, 
...
\vspace{5pt}
\noindent{\bf Semaphores.~} IPC semaphores 
...
\vspace{5pt}
\noindent{\bf Shared File Descriptors.~} 
...
%We expect that the current RPC mechanisms could easily be extended to synchronize a seek pointer among picoprocesses.
...
\vspace{5pt}
\noindent{\bf Shared Memory.~} The Graphene host ABI 
...
%all picoprocesses are on the same host.
...
\vspace{5pt}
\noindent{\bf Failure and Disconnection Tolerance.~}  
...
In the absence of crashes, placing shared state in a given picoprocess introduces the risk that an errant 
...
%After a picoprocess being elected as the leader,
%the leader state,
%including all the allocated IDs and the RPC stream addresses,
%must be recovered. 
%Recollecting the leader state from all the picoprocesses is possible
%but can be inefficient,
%given the new leader may not have knowledge about every picoprocess.
%To simplify the implementation,
%we make the leaders of a namespace periodically serialize their states to disk for later recovery.

%When a picoprocess is sandboxed, it will detect the failure of leader because all of its RPC streams are closed by the reference monitor.
%Once it starts the leadership recovery, it will automatically win because no other picoprocess is sharing the broadcast stream. The procedure can be skipped by informing the sandboxed picoprocess before detaching.
...
\vspace{5pt}
\noindent {\bf Service requests from local state whenever possible.}
...
\vspace{5pt}
\noindent {\bf Lazy discovery and caching improve performance.}  
...
\vspace{5pt}
\noindent {\bf Batched allocation of names minimizes leader workload.}
...
\vspace{5pt}
\noindent {\bf The coordination within a sandbox is often pairwise.}
...
\vspace{5pt}
\noindent {\bf Make RPCs asynchronous whenever possible.} 
...
%this is a mapping to a thread within a specific picoprocess; for a message queue key,
%the mapping might be empty if the queue does not exist, or it may point to the picoprocess storing the pending messages.
%% The general problem underlying each of these coordination APIs is 
%% {\bf namespace management}.  In other words, coordinating picoprocesses need a consistent mapping
%% of names, such as a thread ID or System V message queue ID, to the authoritative picoprocess for that abstraction, if one exists.
