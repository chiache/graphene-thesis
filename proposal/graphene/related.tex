...
This section surveys related work on library OSes,
distributed OSes, and migration and isolation of a process.

\subsection{Previous Library OSes}
...
toward a more virtualization-friendly hardware model~\cite{barham03xen,whitaker02denali, eiraku09outsourcing}.
...
This work extends previous library OSes~\cite{porter11drawbridge,xax,unikernels,baumann13bascule,osv},
...
Bascule~\cite{baumann13bascule} implements a Linux library OS on a variant of the Drawbridge ABI,
...
single-process 
library OS to support a managed language runtime, such as Java, on a bare-metal hypervisor~\cite{osv}.


A number of recent projects have provided a minimal, isolated environment
for web applications to download and execute native code~\cite{nacl,xax,howell13refactoring,gazelle,atlantis}.
...
%%% robustly enforcing browser policy\cite{gazelle};
%%% ensuring that a web page is compatible with the JavaScript, CSS, and other interpreters in the browser~\cite{atlantis};
%%% and simply the ability to push code from the data center onto the client where it makes 
%%% sense~\cite{nacl, embassies, howell13refactoring}.
...
to individual application needs~\cite{kaashoek97exokernel,anderson92libos,cheriton94cache,leslie96nemesis,libra},
...
Kaashoek et al.~\cite{kaashoek97exokernel} identify multi-processing as a problem for an Exokernel libOS,
...
%% In moving services out of the kernel and into user libraries our design resembles a microkernel~\cite{liedtke95sosp, Baron:1985:MOE}.
...
\begin{comment}
Dune~\cite{belay12dune} %merges the concept of a process and VM,
...
User Mode Linux~\cite{user-mode-linux} (UML) executes a Linux kernel inside a process
...
%it is effectively
UML is best described as  an alternative approach to paravirtualization~\cite{barham03xen},
and, unlike a library OS, does not deduplicate functionality.

\subsection{Distributed Coordination APIs}

Distributed operating systems, such as LOCUS~\cite{locus83sosp,fleisch86locus}, Amoeba~\cite{mullender90amoeba,cheriton89naming} and  
Athena~\cite{champine90athena} required a consistent namespace for process identifiers and other IPC abstractions
...
%divide management responsibility across administrative domains~\cite{cheriton89naming}.
...
Plan 9 design~\cite{pike90plan9}, including the unioned view of the host file system
...
Barrelfish~\cite{baumann09barrelfish} argues that multi-core scaling is best served 
...
Cerberus~\cite{song11eurosys} applies similar ideas to coordinate abstractions
...
That said, we expect that systems like Barrelfish~\cite{baumann09barrelfish} 
...
%% {\em within an OS kernel}~\cite{baumann09barrelfish} or {\em across legacy guest OSes}~\cite{song11eurosys}.
%% The focus of this paper is not on multi-core scalability, but on security isolation and compatibility with legacy, multi-process applications.
%% That said, we expect that systems like Barrelfish~\cite{baumann09barrelfish} 
%% could leverage our implementation techniques to efficiently 
%% construct higher-level OS abstractions, such as System V IPC and signals.
...
a non-sibling process was validated by a the parent (``chief'') before a message could leave the clan~\cite{liedtke92clans}.
...
Although this model was abandoned as cumbersome for general-purpose access control~\cite{elphinstone13microkernels},
...
\subsection{Legacy OS support for migration and isolation}

Researchers have added checkpoint and migration support to Linux~\cite{linux-cr}
...
Linux VServer~\cite{vserver},  containers~\cite{bhattiprolu08containers},
and Solaris Zones~\cite{price04zones},
...
with isolated views of namespaces, including process IDs and network interfaces~\cite{lwn-namespaces}.
FreeBSD jails apply a similar approach to augment an isolated {\tt chroot} environment
with other isolated namespaces, including the network and hostname~\cite{jails}.
Similarly, Zap~\cite{osman02zap} migrates groups of process, called a Pod,
...
Shuttle~\cite{shan12shuttle} permits selective violations of strict isolation
