...
Compatibility challenges occur when sharing an application across two distinct system interfaces or reusing an application on a disruptive hardware such as Intel SGX~\cite{intelsgx}.
...
exhaustively porting all applications to new APIs, or preserving all previous APIs for backward compatibility. % toward existing applications.
Since both approaches are resource and time consuming, developers
urgently require a solution to resolve compatibility issues
on a new host OS or hardware,
in order to promptly obtain the benefits of
new technologies.


This thesis demonstrates a library OS approach for reusing applications
on a new host OS or hardware.
The development of a library OS starts with
defining a simplified host ABI,
which contains only OS abstractions essential to applications, including file systems, networking, page management, threading, and process creation.
The host ABI divides the effort
of developing a compatibility layer.
Below the host ABI, a host-specific PAL (platform adaption layer) translates
the host ABI to host system APIs
and enforces security checks on resource sharing.
Above the host ABI, a reusable library OS
emulates the Linux system call table
and namespaces, and only accesses the host ABI when necessary.
...
%Essentially, the complexity of porting is determined by the distinction of \emph{personalities} among platforms. In particular, for a platform as unique as a \libos{}, reusing the applications from commodity operation systems (e.g., Windows and Linux) on a vastly different platform, like the \liboses{},
...
This thesis presents \graphene{}, a \libos{} for reusing unmodified Linux applications,
...
by enumerating host abstractions required for emulating
each Linux system call.
For instance, \graphene{} can emulate multi-process abstractions such as \syscall{fork} and \syscall{execve}
by merely two host abstractions; one is creation of a clean, child process, and the other is a simple pipe-like RPC stream.
Leveraging a distributed coordination model, multiple library OS instances across application processes
collaboratively present a united OS view to the application.



Two main porting targets of the \graphene{} library OS, including a Linux host and a SGX enclave,
each present challenges to enforcing security isolation
upon the threat models.
On a Linux host, \graphene{} presents a sandbox abstraction to isolate mutually-untrusting applications.
Inside a SGX enclave, \graphene{} protects a security-sensitive applications with security checks to validate untrusted system call results.
...
Finally, this thesis presents quantitative measurement to evaluate
the partial compatibility of an OS prototype. Another metric, which evaluates the importance of each API, helps OS developer prioritize which API to port.
...
%provide the personalities of a commodity OS, such as Windows or Linux,
