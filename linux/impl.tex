...
A Linux kernel delivers all hardware exceptions to the Linux PAL
as signals.
If an exception is external to the Linux PAL (from an application or \thelibos{}),
a registered signal handler of the Linux PAL simply calls
...
identifies the internal exception by comparing the faulting program counter
to the mapping address of the Linux PAL,
discovers the recovery information from the stack,
rolls back the context,
and finally returns as failed \hostapi{} to the caller.



The PAL signal handler must avoid further triggering any hardware exceptions from the handler itself, or it can cause a double fault.
\graphene{} ensures that the signal handler is carefully developed
that the handler prevents any memory faults or other exceptions
caused by either defects in the handler code or corrupted inputs from the guest.
An unrecoverable case is stack corruption,
since by default the Linux kernel needs to dump the signal number and the interrupted context on stack
before calling the PAL signal handler.
The Linux PAL can avoid this case by assigning an alternative stack,
or simply kill the \picoproc{} when a double fault happens.
