...
Using the Linux PAL as an example, a PAL developer can estimate the baseline of development effort for porting \thehostabi{} to a new host.
About half of the Linux PAL code turns out to be
generic to different hosts
and fully reusable in different PALs.
The generic PAL components include
...
wrappers for
Linux \linuxapis{};
the major effort for porting \thehostabi{} would be to swap the Linux \linuxapis{}
with any APIs available on the target host.
A PAL developer can expect the implementation of most \hostapis{}
to be fairly straightforward by swapping the Linux \linuxapi{} with similar UNIX-style APIs or even POSIX APIs.
For example, the FreeBSD PAL has reused most of the Linux PAL code,
except for implementing a different system call convention.
