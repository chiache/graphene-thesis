...
The \seccomp{} filter must block unauthorized \linuxapis{}
...
only exporting \palcallnum{} \hostapis{} to each \picoproc{}.
...
The \seccomp{} filter then isolates a \picoproc{} on an unmodified Linux kernel,
...
required for implementing the Linux PAL.
In computer security, the principle of least privilege
restricts each component or layer to only access the resources or abstractions
needed for performing tasks.
The filter limits the execution paths in a Linux kernel
that a \picoproc{} can possibly trigger, to prevent the application running inside the \picoproc{} from exploiting any known or unknown vulnerabilities
on remaining paths.


Although a regular Linux process can also leverage a \seccomp{} filter
for \linuxapi{} restriction,
\graphene{} makes a major contribution
to reduce a massive \linuxapi{} footprint of a large-scale application
to a fixed, simpler \linuxapi{} profile.
Static analysis of applications and libraries
...
running Apache or MySQL in \graphene{} in \graphene{}
leads to the exactly same host \linuxapi{} profile as running any other applications.
For running a wider range of applications
on a system, \graphene{} ensures the OS developers to focus
on a small amount of the corner cases and execution paths to exhaustively test or inspect the kernel code.
...
a \seccomp{} filter implements a compatibility feature
...
The filter blocks \linuxapi{} invoked from outside of the Linux PAL
and delivers a \code{SIGSYS} signal
to the PAL signal handler, which redirects the \linuxapis{} to \thelibos{}.
