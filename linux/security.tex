...
This section starts with goals and threat model,
followed by technical discussion of implementing on a Linux host.
...
On a trusted host,
\graphene{} strengthens the security isolation between applications
by separating 
backward-compatible OS features from security mechanisms.
A Linux kernel exports a wide range of \linuxapis{},
either as a legacy inherited from previous kernel versions or as programmability features of newer kernels.
By moving OS features into a \libos{},
\graphene{} reduces the attack surface of a Linux kernel
to a small amount of corner cases
using \linuxapis{} to implement \thehostabi{}.
...
The complexity of Linux features implemented inside a \libos{} does not extend the attack surface of the host kernel,
unless a Linux feature requires extra \hostapis{} from the host kernel.
With certain amount of effort, a Linux developer
can even carve out a minimal Linux kernel with only the features needed by \graphene{},
similar to converting a Linux kernel into a microkernel.
Otherwise, \graphene{} depends on host-level security mechanisms for restricting a \libos{} from accessing unauthorized \linuxapis{} and resources
in an unmodified Linux kernel.
...
for restricting a whole \picoproc{} from accessing
unauthorized system calls, files, RPC streams, and network addresses.
The Linux PAL uses \hostsyscallnum{} host \linuxapis{}
for implementing \thehostabi{}, including both required and optional \hostapis{}.
A \linuxapi{} filter, such as a SECCOMP filter~\cite{seccomp},
can limit the \linuxapi{} footprint of each \picoproc{}
to only the \hostsyscallnum{} \linuxapis{} needed by the Linux PAL, with additional constraints on the paramters and flags allowed for each \linuxapi{}.
The \linuxapi{} filter
...
Even though the \libos{} spans the multi-process abstractions for an application across multiple \picoprocs{},
a host kernel does not have to examine access to any shared, multi-process
\libos{} states, but still enforce security isolation between sandboxes.
...
a developer specifies the URI prefix and the access type permitted, either as read-only or readable-writable. %, within the target file or directory.
...
Each network rule in a manifest file
...
specify a minimal list of files and network addresses that a \picoproc{} needs to access, and are largely based on existing security policies (e.g., AppArmor profiles, filewall rules).
